...
- **完整词库自动回退**：接入的 WebUI 缺少配套扩展时，自动继续使用插件已下载的本地完整数据包。
- **全库搜索**：提示词分类新增 `全库搜索`，可直接检索完整 TagComplete 数据。
- **一键清空**：`设置 -> 编辑词库` 可一次清空全部自定义/市场词条，不影响 WebUI 源词库和收藏。
- **来源可见**：设置页会显示分类词库与全库搜索当前实际使用的数据来源。
- **节点更稳**：新节点首帧直接使用最近的布局预设；保存设置、缩放或刚放下节点后，LoRA 区域也不会再留下大块空白。
- **支持窄版**：节点最小宽度降至 `640px`，隐藏侧栏后可继续收窄，LoRA 和顶部控件会自动改为窄版排列。
- **摘要阈值可调**：远景摘要默认在画布缩放低于 `50%` 时出现，可在 `设置 -> 布局 / 显示` 中改为 `0-100%`；设为 `0` 可关闭。

完整细节见 [CHANGELOG.md](CHANGELOG.md) 和 [v0.4.23 发布说明](docs/release-notes-v0.4.23.md)。更新后请重启 ComfyUI，并强制刷新浏览器页面。
