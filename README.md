...
- **紧凑 UI**：默认布局更紧凑，侧边栏按模型、生成、图生图和模块分区浏览。
- **远景摘要**：画布缩放低于 68% 时临时收拢为摘要节点，放大后精确恢复原尺寸和面板状态。
- **安全加固**：加强本地配置、请求来源、文件路径、AI 接口和图片解码边界保护。
- **图片与提示词操作优化**：改进图生图入口、Mask 编辑和提示词 Tag 操作体验。

完整细节见 [CHANGELOG.md](CHANGELOG.md) 和 [v0.4.21 发布说明](docs/release-notes-v0.4.21.md)。更新后请重启 ComfyUI，并强制刷新浏览器页面。
