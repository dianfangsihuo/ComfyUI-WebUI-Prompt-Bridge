...
这一版是 `v0.3.0` 之后的布局补丁，重点修复提示词框和 LoRA 卡片区拖拽时的高度分配问题。

默认布局现在会稳定露出正向 Prompt、正向标签和反向 Prompt，同时把 LoRA / Extra Networks 区压缩到只露出一行卡片并固定在节点底部。拖动 `提示词 / LoRA` 分隔条时，LoRA 外壳、左侧分类树和右侧卡片网格会一起撑满可用高度，不会再突然跳成上方一行卡片、下方大片空白。

提示词框、已输入 tag 区、正向/反向标签区和底部 LoRA 区的分隔条也做了同步整理，拖拽分配高度更稳定；双击分隔条仍可恢复默认。反向提示词折叠按钮改成更醒目的 `折叠反向` / `展开反向`。
