...
本次更新修复刷新网页、加载旧工作流或重新新建 Bridge 节点后，LoRA 区底部出现大片空白和抖动的问题，建议所有 v0.4.x 用户更新。

- 保留你手动设置的节点宽高，不会自动恢复默认尺寸。
- LoRA 区会在出现明显底部空白时自动填满面板。
- 刷新、拖拽和加载旧工作流后的布局更稳定。
- 更新后请重启 ComfyUI，并强制刷新浏览器页面。

## v0.4.x 更新概要

- Bridge 从提示词编辑节点升级为“提示词工作台 + 功能助手侧栏”，基础提示词、模型切换、尺寸快捷、布局预设和 LoRA 浏览默认可用。
- 区域控制、Styles、ADetailer、ControlNet、SAM/Inpaint、Mask、图生图和放大修复等高级功能可以在 `设置` 里按需显示。
- 支持一键构建外置高级节点，并尽量自动连接到现有工作流。
- LoRA 浏览、WebUI 接入、预览图、metadata、分类和多级目录浏览持续优化。
- 节点布局、画布拖动/缩放、右下角拖拽、窄高布局和旧工作流尺寸恢复更加稳定。
- 提示词 tag 编辑支持保留换行、中文翻译、自动补全、收藏、Styles、提示词市场和多选拖拽。
