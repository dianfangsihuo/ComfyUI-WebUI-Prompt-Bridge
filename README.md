...
- **联网翻译可诊断**：服务失败、网络/SSL 错误、模块缺失、路径冲突和配置问题会显示具体原因。
- **提示词按钮有反馈**：单个 Tag、整段翻译和自动翻译都会显示进行中、完成或原文保留状态。
- **失败可恢复**：临时翻译失败不再缓存为空，网络恢复后可以重新请求。

更新后请**重启 ComfyUI**，再按 `Ctrl+F5` 刷新页面。详情见 [v0.4.33 发布说明](docs/release-notes-v0.4.33.md) 和 [更新日志](CHANGELOG.md)。
...
- **精确排序与微调**：换行后的 Tag 也能按二维位置拖放；可多选整体移动，并用按钮或 `Alt+←/→` 前后移动一位。
- **自动补全**：节点随包自带完整 Danbooru 主 Tag 数据和中文解释；输入字母即可看到 Tag、中文含义和热度数量，不需要另外安装 WebUI 或 TagComplete。
- **收藏提示词**：常用角色、服装、画风、表情、动作可以收藏起来反复使用；支持父级/子级分类、拖放归类、全库搜索和每页 100 条分页。
...
- **模型快速切换**：在 Bridge 面板里选择 `diffusion_models` 中的分体 UNET 或任意整合 checkpoint；切换 UNET 时沿用当前 Text Encoder/VAE，并保护节点与连线。
- **从图片读取 Prompt**：点击主节点顶部的“读图提示词”（右侧“布局与工具”里也保留紧凑入口），从 PNG/JPEG/WebP 的原始 metadata 读回 Bridge 正反 Prompt，并兼容 A1111 parameters 回退；应用前可选择候选和覆盖方向。
...
- **开关 ON**：使用分体模型，也就是 `UNET + CLIP + VAE`。模型下拉会列出 `diffusion_models` 中的 UNET；选择后只更换当前 Bridge 对应的 `UNETLoader.unet_name`，继续沿用现有 CLIP 和 VAE。
- **开关 OFF**：使用普通整合 checkpoint，也就是 `CheckpointLoaderSimple`。适合 SD1.5、SDXL、Pony、Illustrious、Qwen-Rapid-AIO 这类整合模型。

如果当前 Bridge 无法唯一定位一个可用的 `UNETLoader`，切换会被拒绝并显示原因，不会批量修改工作流中的全部 Loader。分体 UNET、Text Encoder 和 VAE 的架构兼容性仍需由用户确认。
...
欢迎提交可复现的 Bug、性能数据、文档和代码。请先阅读 [CONTRIBUTING.md](CONTRIBUTING.md)，并使用仓库提供的 Issue / PR 模板。收藏性能问题请使用合成数据说明数量、操作步骤和测量结果，不要上传真实收藏或敏感配置。

大型功能建议先开 Issue 对齐范围；一个 PR 尽量只处理一个主题。代码合入后会保留贡献者署名，问题报告和设计贡献也会在更新日志中致谢。
