...
Bridge 现在更像一个完整的提示词工作台：常用的提示词编辑、模型切换、LoRA 浏览和布局控制默认可用；区域控制、Styles、ADetailer、ControlNet、SAM/Inpaint、Mask、图生图和放大修复可以在 `设置` 里按需打开，并支持一键构建对应外置节点。
...
本次更新重点修复 Bridge 面板布局保存和拖拽稳定性，建议所有 v0.4.x 用户更新。

- 面板布局状态现在会随 workflow 保存，重启或重新加载后能恢复侧栏、Prompt/LoRA 分区、提示词区高度和 LoRA 滚动位置。
- 修复 LoRA 卡片列表在刷新、勾选或重新渲染后自动上移的问题。
- 修复正向/反向提示词区域拖动后被自适应布局拉回的问题。
- 区分“节点宽/高”和“画布宽/高”，区域画布尺寸调整不再影响 Bridge 节点外框。
- 收敛重复的布局自愈路径，避免画布拖动、区域尺寸输入、普通自适应布局改写用户手动尺寸。
...
它现在集中处理提示词/反向词编辑、LoRA 浏览与真实加载、模型切换、布局预设、区域控制、图生图和常用高级模块入口。近期更新主要围绕三点：旧工作流兼容、节点布局稳定、LoRA/提示词编辑手感。
