...
- 主节点顶部“读图提示词”可从 PNG/JPEG/WebP 读取 Bridge 正向/反向 Prompt（右侧“布局与工具”保留紧凑入口）。存在多个候选时必须手动选择，正向和反向可分别勾选；确认前、取消后或解析失败时不改当前 Prompt。
- 修复 LoRA 区域折叠/展开时自动布局改写 Prompt/LoRA 分栏高度的问题。折叠只改变临时可见高度，展开和工作流序列化都会恢复原先保存的分栏。
