...
const PROMPT_ONLY_NODES = {
    WebUIPromptBridgePositivePrompt: {
        kind: "positive",
        widgetName: "positive_prompt",
        label: "正向提示词",
        placeholder: "输入正向提示词，键入 tag 时会自动补全…",
        color: "#315145",
        bgcolor: "#172920",
    },
    WebUIPromptBridgeNegativePrompt: {
        kind: "negative",
        widgetName: "negative_prompt",
        label: "负向提示词",
        placeholder: "输入负向提示词，键入 tag 时会自动补全…",
        color: "#543b46",
        bgcolor: "#2b1920",
    },
};
...
const SLOT_LABEL_OVERLAY_FRAME_INTERVAL_MS = 1000 / 30;
const SLOT_LABEL_OVERLAY_HEALTH_INTERVAL_MS = 750;
const SLOT_LABEL_OVERLAY_RECT_REFRESH_MS = 1000;
...
const BRIDGE_LAYOUT_STATE_VERSION = 1;
...
const BRIDGE_LAYOUT_STORAGE_KEYS = Object.freeze({
    top_row_height: "webui-bridge-toprow-height",
    extra_height: "webui-bridge-extra-height",
    extra_collapsed: "webui-bridge-extra-collapsed",
    sidebar_width: "webui-bridge-action-column-width",
    sidebar_collapsed: "webui-bridge-action-column-collapsed",
    negative_collapsed: "webui-bridge-negative-collapsed",
    positive_textarea_height: "webui-bridge-textarea-height-prompt",
    negative_textarea_height: "webui-bridge-textarea-height-negative-prompt",
    positive_chip_height: "webui-bridge-chip-size-positive",
    negative_chip_height: "webui-bridge-chip-size-negative",
    positive_tag_height: "webui-bridge-aio-panel-height-positive",
...
const BRIDGE_LAYOUT_STATE_NUMBER_SPECS = Object.freeze({
    ...
    sidebar_width: [ACTION_SIDEBAR_MIN_WIDTH, ACTION_SIDEBAR_MAX_WIDTH],
    positive_textarea_height: [48, 520],
    negative_textarea_height: [48, 520],
    positive_chip_height: [PROMPT_CHIPS_MIN_HEIGHT, 520],
    negative_chip_height: [PROMPT_CHIPS_MIN_HEIGHT, 520],
    positive_tag_height: [AIO_POSITIVE_MIN_HEIGHT, 680],
    negative_tag_height: [AIO_NEGATIVE_MIN_HEIGHT, 680],
    lora_scroll_top: [0, 100000],
});
const BRIDGE_LAYOUT_STATE_BOOLEAN_KEYS = Object.freeze([
    "extra_collapsed",
    "sidebar_collapsed",
    "negative_collapsed",
]);
const BRIDGE_LAYOUT_MIGRATED_STORAGE_KEYS = Object.freeze([
    BRIDGE_LAYOUT_STORAGE_KEYS.top_row_height,
    BRIDGE_LAYOUT_STORAGE_KEYS.extra_height,
    BRIDGE_LAYOUT_STORAGE_KEYS.extra_collapsed,
    BRIDGE_LAYOUT_STORAGE_KEYS.negative_collapsed,
    BRIDGE_LAYOUT_STORAGE_KEYS.positive_chip_height,
    BRIDGE_LAYOUT_STORAGE_KEYS.negative_chip_height,
    BRIDGE_LAYOUT_STORAGE_KEYS.positive_tag_height,
    BRIDGE_LAYOUT_STORAGE_KEYS.negative_tag_height,
    BRIDGE_LAYOUT_STORAGE_KEYS.lora_scroll_top,
]);
...
const DEFAULT_BRIDGE_SETTINGS = {
    ...
    zoom_summary_threshold: BRIDGE_LOW_ZOOM_THRESHOLD,
    ui_visibility: {},
};
const BRIDGE_LAYOUT_PRESET_CACHE_KEY = "webui-bridge-layout-preset-cache-v1";
const BRIDGE_FRESH_PANEL_SIZE_BY_PRESET = Object.freeze({
    default: [DEFAULT_PANEL_WIDTH, DEFAULT_PANEL_HEIGHT],
    compact: [920, 680],
    roomy: [1380, 1180],
    positive_focus: [1180, 760],
    minimal_lora: [940, 560],
});
const COMPACT_EXTRA_INITIAL_HEIGHT = 152;
const BRIDGE_FRESH_SECTION_SIZE_BY_PRESET = Object.freeze({
    default: { top: 620, extra: 452 },
    compact: { top: 420, extra: COMPACT_EXTRA_INITIAL_HEIGHT },
    roomy: { top: 840, extra: EXTRA_NETWORKS_MIN_HEIGHT },
    positive_focus: { top: 500, extra: EXTRA_NETWORKS_MIN_HEIGHT },
    minimal_lora: { top: 460, extra: 42 },
});
...
function normalizeBridgeLowZoomThreshold(value, fallback = BRIDGE_LOW_ZOOM_THRESHOLD) {
    const fallbackNumber = Number(fallback);
    const safeFallback = Number.isFinite(fallbackNumber)
        ? Math.min(BRIDGE_LOW_ZOOM_THRESHOLD_MAX, Math.max(BRIDGE_LOW_ZOOM_THRESHOLD_MIN, fallbackNumber))
        : BRIDGE_LOW_ZOOM_THRESHOLD;
    if (typeof value === "string" && !value.trim()) return safeFallback;
    const numeric = Number(value);
    if (!Number.isFinite(numeric)) return safeFallback;
    return Math.round(
        Math.min(BRIDGE_LOW_ZOOM_THRESHOLD_MAX, Math.max(BRIDGE_LOW_ZOOM_THRESHOLD_MIN, numeric)) * 100,
    ) / 100;
}

function bridgeLowZoomThreshold(node) {
    return normalizeBridgeLowZoomThreshold(
        node?.__webuiBridgeLowZoomThreshold ?? node?.__webuiBridgePanel?.__webuiBridgeLowZoomThreshold,
    );
}

function setBridgeLowZoomThreshold(node, value, panel = null) {
    const threshold = normalizeBridgeLowZoomThreshold(value);
    if (node) node.__webuiBridgeLowZoomThreshold = threshold;
    const targetPanel = panel || node?.__webuiBridgePanel;
    if (targetPanel) targetPanel.__webuiBridgeLowZoomThreshold = threshold;
    return threshold;
}

function setGraphBridgeLowZoomThreshold(value) {
    const threshold = normalizeBridgeLowZoomThreshold(value);
    for (const graphNode of getGraphNodes()) {
        if (graphNode?.type === "WebUIPromptBridge") setBridgeLowZoomThreshold(graphNode, threshold);
    }
...
function normalizeBridgeSettings(settings = {}) {
    ...
    for (const [key, value] of Object.entries(settings || {})) {
        ...
        } else if (key === "zoom_summary_threshold") {
            normalized[key] = normalizeBridgeLowZoomThreshold(value, normalized[key]);
...
function normalizeBridgeLayoutPreset(preset) {
    return SETTING_CHOICES.layout_preset.includes(preset)
        ? preset
        : DEFAULT_BRIDGE_SETTINGS.layout_preset;
}

function readBridgeInitialLayoutPreset() {
    return normalizeBridgeLayoutPreset(readLocalString(
        BRIDGE_LAYOUT_PRESET_CACHE_KEY,
        DEFAULT_BRIDGE_SETTINGS.layout_preset,
    ));
}

function rememberBridgeLayoutPreset(preset) {
    const normalized = normalizeBridgeLayoutPreset(preset);
    writeLocalString(BRIDGE_LAYOUT_PRESET_CACHE_KEY, normalized);
    return normalized;
}

function freshBridgePanelSizeForPreset(preset) {
    const normalized = normalizeBridgeLayoutPreset(preset);
    return [...(BRIDGE_FRESH_PANEL_SIZE_BY_PRESET[normalized] || BRIDGE_FRESH_PANEL_SIZE_BY_PRESET.compact)];
}
...
function bridgeTrySetPointerCapture(target, pointerId) {
    try {
        target?.setPointerCapture?.(pointerId);
    } catch {
        // Pointer capture can race with a cancelled pointer on touch/relay paths.
    }
}
...
function persistResizeTargetHeight(target, storageKey) {
    const key = resizeTargetKey(target, storageKey);
    const height = resizeTargetLayoutHeight(target);
    if (key && Number.isFinite(height) && height > 0) writeLocalNumber(key, height);
}
...
function createHeightResizeGrip(target, storageKey, { min = 48, max = 640, title = "拖动调整高度，双击恢复", onDragStart = null, onDragMove = null, onDragEnd = null } = {}) {
    const grip = el("div", {
        ...
        onpointerdown: (event) => {
            ...
            let moved = false;
            bridgeTrySetPointerCapture(grip, event.pointerId);
            grip.classList.add("dragging");
            const onMove = (moveEvent) => {
                const delta = (moveEvent.clientY - startY) / viewportScale;
                moved = true;
                onDragMove?.(moveEvent, { target, delta, startHeight });
                setResizeTargetHeight(target, storageKey, startHeight + delta, { min, max, persist: false });
            };
            const onUp = () => {
                grip.classList.remove("dragging");
                if (moved) persistResizeTargetHeight(target, storageKey);
                onDragEnd?.({ target, grip });
        ...
        onpointerdown: (event) => {
            ...
            const onMove = (moveEvent) => {
                ...
                if (!afterVisible || total <= beforeMin + afterMin) {
                    setResizeTargetHeight(before, beforeKey, rawBefore, { min: beforeMin, max: beforeMax, persist: false });
                    return;
                }
                ...
                setResizeTargetHeight(before, beforeKey, nextBefore, { min: minBefore, max: maxBefore, persist: false });
                if (options.fillAfter) {
                    after.style.height = "";
                    after.style.flex = options.afterResetFlex || "1 1 0";
                } else {
                    const nextAfter = total - nextBefore;
                    setResizeTargetHeight(after, afterKey, nextAfter, { min: afterMin, max: afterMax, persist: false });
            ...
            const onUp = () => {
                grip.classList.remove("dragging");
                if (moved) {
                    persistResizeTargetHeight(before, beforeKey);
                    if (afterVisible) {
                        if (options.fillAfter) resetResizeTargetHeight(after, afterKey, options.afterResetFlex || "1 1 0");
                        else persistResizeTargetHeight(after, afterKey);
                    }
                }
                options.onDragEnd?.();
...
function setSidebarWidth(topRow, storageKey, width, options = {}) {
    ...
    const max = Number.isFinite(options.resolvedMax) ? options.resolvedMax : sidebarWidthMax(topRow, options);
    const nextWidth = clampNumber(width, min, max);
    topRow.style.setProperty("--webui-bridge-sidebar-width", `${nextWidth}px`);
    if (storageKey && options.persist !== false) writeLocalNumber(storageKey, nextWidth);
    return nextWidth;
}
...
function applyStoredSidebarWidth(topRow, storageKey, options = {}) {
    ...
    setSidebarWidth(topRow, storageKey, width, { ...options, persist: false });
}

function resetSidebarWidth(topRow, storageKey, options = {}) {
    if (!topRow) return;
    if (storageKey) clearLocalValue(storageKey);
    const width = setSidebarWidth(topRow, "", options.defaultWidth ?? ACTION_SIDEBAR_DEFAULT_WIDTH, options);
...
function startSidebarWidthDrag(handle, topRowRef, storageKey, options, event) {
    ...
    const dragMax = sidebarWidthMax(topRow, options);
    const dragOptions = { ...options, persist: false, resolvedMax: dragMax };
    let moved = false;
    let committedWidth = startWidth;
    handle.classList.add("dragging");
    document.body?.classList?.add("webui-bridge-resizing-sidebar");
    bridgeTrySetPointerCapture(handle, event.pointerId);
    const onMove = (moveEvent) => {
        const delta = (moveEvent.clientX - startX) / viewportScale;
        if (!moved && Math.abs(delta) < 3) return;
        moved = true;
        committedWidth = setSidebarWidth(topRow, storageKey, startWidth - delta, dragOptions) ?? committedWidth;
    };
    const onUp = () => {
        ...
        document.removeEventListener("pointercancel", onUp, true);
        if (moved) {
            if (storageKey) writeLocalNumber(storageKey, committedWidth);
            options.onCommit?.(committedWidth);
        }
    };
    document.addEventListener("pointermove", onMove, true);
    document.addEventListener("pointerup", onUp, true);
...
function bridgeLayoutStateNumber(source, key, min, max) {
    const value = Number(source?.[key]);
    if (!Number.isFinite(value)) return null;
    return clampNumber(Math.round(value), min, max);
}

function bridgeLayoutStateBoolean(source, key) {
...
function normalizeBridgeLayoutState(source) {
    if (!source || typeof source !== "object") return null;
    const state = { version: BRIDGE_LAYOUT_STATE_VERSION };
    for (const [key, [min, max]] of Object.entries(BRIDGE_LAYOUT_STATE_NUMBER_SPECS)) {
        const value = bridgeLayoutStateNumber(source, key, min, max);
        if (value !== null) state[key] = value;
    }
    for (const key of BRIDGE_LAYOUT_STATE_BOOLEAN_KEYS) {
        const value = bridgeLayoutStateBoolean(source, key);
        if (value !== null) state[key] = value;
    }
...
function hasBridgeLayoutState(source) {
    return Boolean(normalizeBridgeLayoutState(source));
}
...
function bridgePanelSizesMatch(first, second, tolerance = 2) {
    const firstValues = bridgePanelSizeValues(first);
    const secondValues = bridgePanelSizeValues(second);
    if (!firstValues || !secondValues) return false;
    return Math.abs(firstValues[0] - secondValues[0]) <= tolerance &&
        Math.abs(firstValues[1] - secondValues[1]) <= tolerance;
}

function looksLikeCurrentDefaultBridgePanelSize(size) {
    const values = bridgePanelSizeValues(size);
    if (!values) return false;
    return Math.abs(values[0] - DEFAULT_PANEL_WIDTH) <= 8 &&
        Math.abs(values[1] - DEFAULT_PANEL_HEIGHT) <= 8;
}

function shouldUseLiveBridgePanelSize(node, liveSize, savedSize) {
    ...
    if (!bridgeConfiguredSizeLockActive(node)) return true;
    if (!looksLikeSavedUserBridgePanelSize(savedSize)) return true;
    if (bridgePanelSizesMatch(liveSize, savedSize)) return true;
    return savedLooksDefault && !liveLooksDefault;
}

function rememberBridgePanelSize(node, size, options = {}) {
    const desired = normalizeBridgePanelSize(size);
    node.__webuiBridgeDesiredSize = [...desired];
    if (looksLikeSavedUserBridgePanelSize(desired)) {
        node.__webuiBridgeSavedPanelSize = [...desired];
        if (options.clearConfiguredLock) {
...
function resolveBridgePanelSize(node) {
    if (bridgeConfiguredSizeLockActive(node)) {
        return normalizeBridgePanelSize(node.__webuiBridgeSavedPanelSize);
    }
    if (looksLikeSavedUserBridgePanelSize(node?.size)) {
...
function resolveBridgePanelSizeForLiveState(node, options = {}) {
    ...
    const liveSize = looksLikeSavedUserBridgePanelSize(node?.size)
        ...
        : null;
    const savedSize = looksLikeSavedUserBridgePanelSize(node?.__webuiBridgeSavedPanelSize)
        ? normalizeBridgePanelSize(node.__webuiBridgeSavedPanelSize)
        : null;
    if (liveSize && shouldUseLiveBridgePanelSize(node, liveSize, savedSize)) {
        const liveDiffersFromSaved = savedSize && !bridgePanelSizesMatch(liveSize, savedSize);
        return rememberBridgePanelSize(node, liveSize, {
            clearConfiguredLock: Boolean(options.clearConfiguredLock && liveDiffersFromSaved),
        });
    }
...
function normalizeBridgePanelSize(size, fallback = [DEFAULT_PANEL_WIDTH, DEFAULT_PANEL_HEIGHT]) {
    if (shouldRepairBridgePanelSize(size)) {
...
function drawBridgeExternalSlotLabels(node, ctx) {
    ...
    const lowZoomThreshold = bridgeLowZoomThreshold(node);
    if (lowZoomThreshold > 0 && scale < lowZoomThreshold) return;
...
function removeBridgeSlotLabelOverlay(node) {
    ...
    if (node.__webuiBridgeCanvasScaleSettleTimer) {
        window.clearTimeout?.(node.__webuiBridgeCanvasScaleSettleTimer);
        node.__webuiBridgeCanvasScaleSettleTimer = 0;
    }
    node.__webuiBridgeLastCanvasScale = null;
...
function installBridgeSlotLabelOverlay(node) {
    ...
    let canvasElement = document.querySelector("#graph-canvas");
    let canvasRect = null;
    let canvasRectSignature = "";
    let lastViewSignature = "";
    let nextVisualUpdateAt = 0;
    let nextHealthCheckAt = 0;
    let nextRectRefreshAt = 0;
    let labels = [];
    let labelSignature = "";
    let labelsByKey = new Map();
    ...
    const update = (frameTime = 0) => {
        const now = Number(frameTime) || performance?.now?.() || Date.now();
        ...
        if (now >= nextHealthCheckAt) {
            nextHealthCheckAt = now + SLOT_LABEL_OVERLAY_HEALTH_INTERVAL_MS;
            if (!node.graph || !bridgeNodeIsInCurrentGraph(node)) {
                ...
                if (node.__webuiBridgeSlotLabelStaleFrames > 1) {
                    removeBridgeSlotLabelOverlay(node);
                    cleanupBridgeSlotLabelOverlays();
                    return;
                }
            ...
            cleanupBridgeSlotLabelOverlays([overlay]);
            if (!canvasElement?.isConnected) canvasElement = document.querySelector("#graph-canvas");
            const nextLabels = bridgeVisibleSlotLabels(node);
            const nextLabelSignature = nextLabels.map((item) => item.key).join("|");
            if (nextLabelSignature !== labelSignature) {
                labels = nextLabels;
                labelSignature = nextLabelSignature;
                lastViewSignature = "";
            }
        }
        if (now < nextVisualUpdateAt) {
            scheduleNextUpdate();
            return;
        }
        nextVisualUpdateAt = now + SLOT_LABEL_OVERLAY_FRAME_INTERVAL_MS;
        const canvas = app?.canvas;
        const scale = canvas?.ds?.scale || 1;
        const previousScale = Number(node.__webuiBridgeLastCanvasScale);
        if (!Number.isFinite(previousScale) || Math.abs(previousScale - scale) > 0.0005) {
            node.__webuiBridgeLastCanvasScale = scale;
            window.clearTimeout?.(node.__webuiBridgeCanvasScaleSettleTimer);
            node.__webuiBridgeCanvasScaleSettleTimer = window.setTimeout?.(() => {
                node.__webuiBridgeCanvasScaleSettleTimer = 0;
                const livePanel = node.__webuiBridgePanel;
                if (!livePanel?.isConnected) return;
                livePanel.__webuiBridgeSettleManualResizeLayout?.();
            }, 140) || 0;
        }
        const lowZoomThreshold = bridgeLowZoomThreshold(node);
        const lowZoom = lowZoomThreshold > 0 && scale < lowZoomThreshold;
        const offset = canvas?.ds?.offset || [];
        const nodePosition = node.pos || [];
        const nodeSize = node.size || [];
        const viewSignature = [
            Math.round(scale * 10000),
            Math.round(Number(offset[0] || 0) * 10),
            Math.round(Number(offset[1] || 0) * 10),
            Math.round(Number(nodePosition[0] || 0) * 10),
            Math.round(Number(nodePosition[1] || 0) * 10),
            Math.round(Number(nodeSize[0] || 0) * 10),
            Math.round(Number(nodeSize[1] || 0) * 10),
            Math.round(lowZoomThreshold * 100),
            labelSignature,
            window.scrollX || 0,
            window.scrollY || 0,
        ].join(":");
        const viewChanged = viewSignature !== lastViewSignature;
        const refreshCanvasRect = viewChanged || !canvasRect || now >= nextRectRefreshAt;
        if (refreshCanvasRect) {
            const nextRect = canvasElement?.getBoundingClientRect?.() || null;
            const nextRectSignature = nextRect
                ? [nextRect.left, nextRect.top, nextRect.width, nextRect.height].map((value) => Math.round(value * 10)).join(":")
                : "";
            const rectChanged = nextRectSignature !== canvasRectSignature;
            canvasRect = nextRect;
            canvasRectSignature = nextRectSignature;
            nextRectRefreshAt = now + SLOT_LABEL_OVERLAY_RECT_REFRESH_MS;
        ...
        if (!canvasRect || lowZoom) {
            ...
            if (labelSignature !== node.__webuiBridgeSlotLabelSignature) {
                ...
                labelsByKey = new Map([...overlay.children].map((child) => [child.dataset.key, child]));
                node.__webuiBridgeSlotLabelSignature = labelSignature;
            }
            for (const item of labels) {
                ...
                const label = labelsByKey.get(item.key);
                if (!label) continue;
                ...
                const left = `${Math.round(clientX)}px`;
                const top = `${Math.round(clientY)}px`;
...
function syncBridgeDomWidgetWrapper(panel, width, height, options = {}) {
    ...
    const storedOptions = panel.__webuiBridgeWidgetSizeOptions || {};
    const minWidth = Number.isFinite(options.minWidth)
        ? options.minWidth
        : Number.isFinite(storedOptions.minWidth) ? storedOptions.minWidth : DOM_WIDGET_CONTENT_MIN_WIDTH;
    const minHeight = Number.isFinite(options.minHeight)
        ? options.minHeight
        : Number.isFinite(storedOptions.minHeight) ? storedOptions.minHeight : 280;
    panel.__webuiBridgeWidgetSizeOptions = { minWidth, minHeight };
    const wrapperWidth = Math.max(minWidth, Number(width) || panel.offsetWidth || DEFAULT_PANEL_WIDTH);
    const wrapperHeight = Math.max(minHeight, Number(height) || panel.offsetHeight || DEFAULT_PANEL_HEIGHT);
    ...
    for (const root of roots) {
        root.classList?.add?.("webui-bridge-dom-widget-shell");
        setImportant(root, "width", "100%");
        setImportant(root, "max-width", "none");
        setImportant(root, "min-width", "0");
    ...
    if (widgetHost) {
        setImportant(widgetHost, "display", "grid");
        setImportant(widgetHost, "grid-template-rows", "minmax(0, 1fr)");
    }
...
function bridgeCanvasHasPendingNodePlacement() {
    ...
    return canvas.state?.ghostNodeId !== undefined && canvas.state.ghostNodeId !== null;
}

function bridgeCanvasPointerIsActive(canvas = app?.canvas) {
    if (!canvas) return false;
    return Boolean(canvas.pointer?.isDown || canvas.dragging_canvas || canvas.isDragging || canvas._dragZoomStart);
}

function bridgeCanvasEventPathIncludes(event, element) {
    if (!event || !element) return false;
    if (event.target === element || element.contains?.(event.target)) return true;
    return Boolean(event.composedPath?.().includes(element));
}

function recoverBridgeStaleCanvasPointer(event) {
    const canvas = app?.canvas;
    if (!canvas?.pointer?.isDown) return false;
    if (event?.buttons) return false;
    try {
        if (typeof canvas.processMouseCancel === "function") {
            canvas.processMouseCancel(event);
        } else {
        ...
        console.warn("[WebUI Prompt Bridge] Failed to recover stale ComfyUI canvas pointer", error);
        return false;
    }
}

function installBridgeCanvasPointerRecovery() {
    ...
    const recover = (event) => {
        ...
        recoverBridgeStaleCanvasPointer(event);
    };
    document.addEventListener("pointermove", recover, true);
    document.addEventListener("mousemove", recover, true);
    document.addEventListener("mouseup", recover, true);
    document.addEventListener("pointerup", recover, true);
    document.addEventListener("pointercancel", recover, true);
    document.addEventListener("keyup", (event) => {
        if (event.key === "Escape") recoverBridgeStaleCanvasPointer();
    }, true);
}

function bridgePanelTargetOwnsPointer(target, handleSelector) {
    ...
    const localSelector = [
        ...
        "[role='radio']",
        "[role='switch']",
        "[tabindex]:not(.webui-bridge-panel):not(.webui-bridge-toprow):not(.webui-bridge-extra)",
        ".webui-bridge-card",
        ".webui-bridge-card-trigger",
        ".webui-bridge-tree-item",
        ".webui-bridge-custom-tag-row",
        ".webui-bridge-tag-chip",
        ".webui-bridge-prompt-chip",
        ".webui-bridge-prompt-label",
        ".webui-bridge-extra-head",
        ".webui-bridge-network-controls",
        ".webui-bridge-model-controls",
    ].filter(Boolean).join(",");
    const pointerOwner = target.closest?.(localSelector);
    if (!pointerOwner) return false;
    return !panel || panel.contains(pointerOwner);
}

function forwardBridgeCanvasPointer(event, methodName) {
    const canvas = app?.canvas;
    if (typeof canvas?.[methodName] !== "function") return false;
    try {
        canvas[methodName](event);
        if (methodName === "processMouseDown") {
            try {
                Object.defineProperty(event, "__webuiBridgeCanvasPointerForwarded", { value: true, configurable: true });
            } catch {
                event.__webuiBridgeCanvasPointerForwarded = true;
            }
        }
        return true;
    } catch (error) {
        console.warn(`[WebUI Prompt Bridge] Failed to forward ${event.type} to ComfyUI canvas`, error);
        return false;
    }
}
...
function installBridgeCanvasPointerRelay(startEvent) {
    const canvas = app?.canvas;
    const canvasElement = canvas?.canvas;
    if (!canvas || !canvasElement) return;
    const previousRelay = document.__webuiBridgeCanvasPointerRelay;
    previousRelay?.cleanup?.();
    const pointerId = startEvent.pointerId;
    const isPointerRelay = startEvent.type.startsWith("pointer");
    const matchesActivePointer = (event) => {
        if (isPointerRelay && event.pointerId !== pointerId) return false;
        return true;
    };
    const shouldForward = (event) => {
        if (!matchesActivePointer(event)) return false;
        if (bridgeCanvasEventPathIncludes(event, canvasElement)) return false;
        return bridgeCanvasPointerIsActive(canvas);
    };
    const relayMove = (event) => {
        if (!shouldForward(event)) return;
        forwardBridgeCanvasPointer(event, "processMouseMove");
    };
    const relayUp = (event) => {
        if (matchesActivePointer(event) && !bridgeCanvasEventPathIncludes(event, canvasElement) && bridgeCanvasPointerIsActive(canvas)) {
            forwardBridgeCanvasPointer(event, "processMouseUp");
        }
        cleanup();
    };
    const relayCancel = (event) => {
        if (matchesActivePointer(event) && bridgeCanvasPointerIsActive(canvas)) {
            forwardBridgeCanvasPointer(event, "processMouseCancel");
        }
        cleanup();
    };
    const cleanup = () => {
        document.removeEventListener("pointermove", relayMove, true);
        document.removeEventListener("pointerup", relayUp, true);
        document.removeEventListener("pointercancel", relayCancel, true);
        document.removeEventListener("mousemove", relayMove, true);
        document.removeEventListener("mouseup", relayUp, true);
        window.removeEventListener("blur", relayCancel, true);
        if (document.__webuiBridgeCanvasPointerRelay?.cleanup === cleanup) {
            document.__webuiBridgeCanvasPointerRelay = null;
        }
    };
    document.__webuiBridgeCanvasPointerRelay = { cleanup };
    if (isPointerRelay) {
        document.addEventListener("pointermove", relayMove, true);
        document.addEventListener("pointerup", relayUp, true);
        document.addEventListener("pointercancel", relayCancel, true);
    } else {
        document.addEventListener("mousemove", relayMove, true);
        document.addEventListener("mouseup", relayUp, true);
    }
...
function installBridgeDocumentCanvasPointerCapture(handleSelector) {
    ...
    const captureCanvasPointer = (event) => {
        const target = event.target;
        if (!(target instanceof Element)) return;
        const panel = target.closest?.(".webui-bridge-panel");
        if (!panel || target.closest?.(handleSelector)) return;
        const isPointerStart = event.type === "pointerdown" || (!window.PointerEvent && event.type === "mousedown");
        if (!isPointerStart) return;
        const wantsCanvas = bridgeCanvasHasPendingNodePlacement() || event.button === 1 || event.buttons === 4;
        if (!wantsCanvas) return;
        installBridgeCanvasPointerRelay(event);
        if (forwardBridgePendingNodePlacementPointer(event) ||
            forwardBridgePanelPointerDown(event, handleSelector)) {
            stopBridgeCanvasOwnedPointer(event);
        }
    };
    document.addEventListener("pointerdown", captureCanvasPointer, true);
    document.addEventListener("mousedown", captureCanvasPointer, true);
}
...
function installBridgePanelCanvasEventGuards(panel) {
    ...
    installBridgeCanvasPointerRecovery();
    ...
    installBridgeDocumentCanvasPointerCapture(handleSelector);
    const captureCanvasPointer = (event) => {
    ...
    const stopCanvasPointer = (event) => {
        if (event.target?.closest?.(handleSelector)) return;
        if (event.type === "pointerdown" || (!window.PointerEvent && event.type === "mousedown")) {
            installBridgeCanvasPointerRelay(event);
        }
        if (event.type === "pointerdown" || (!window.PointerEvent && event.type === "mousedown")) {
            if (forwardBridgePendingNodePlacementPointer(event)) {
            ...
            if (forwardBridgePanelPointerDown(event, handleSelector)) {
                event.stopPropagation();
                return;
            }
        }
    ...
    panel.addEventListener("pointerdown", captureCanvasPointer, true);
    panel.addEventListener("mousedown", captureCanvasPointer, true);
...
function repairBridgePanelLayoutAfterWorkflow(node, reason = "workflow") {
    if (!node || node.type !== TARGET_NODE) return;
    const run = () => {
        if (!isBridgePanelUsable(node)) {
            scheduleBridgePanelInstall(node);
            return;
        }
        const widget = node.widgets?.find((item) => item?.name === "webui_prompt_frontend" && item.element === node.__webuiBridgePanel);
        const panel = node.__webuiBridgePanel;
        const desired = resolveBridgePanelSizeForLiveState(node, { clearConfiguredLock: true });
        pinBridgeDomWidgetToTop(node, widget);
        if (Math.round(Number(node.size?.[0] || 0)) !== desired[0] ||
            Math.round(Number(node.size?.[1] || 0)) !== desired[1]) {
            node.__webuiBridgeApplyingConfiguredSize = bridgeConfiguredSizeLockActive(node);
            try {
                node.setSize?.([...desired]);
            } finally {
                node.__webuiBridgeApplyingConfiguredSize = false;
            }
        }
        node.__webuiBridgeApplyDomWidgetSize?.(desired[0], desired[1]);
        panel?.__webuiBridgeScheduleAdaptiveLayout?.();
...
function showBridgeMaskEditor(node, imageName, maskName, onSaved, widgetNames = {}) {
    ...
    maskCanvas.addEventListener("pointerdown", (event) => {
        if (!editorReady()) return;
        drawing = true;
        bridgeTrySetPointerCapture(maskCanvas, event.pointerId);
        paintAt(event);
    });
...
function getAppGraphSafe() {
    ...
    const graphReady = typeof app.isGraphReady === "function" ? app.isGraphReady() : app.isGraphReady;
    if (graphReady === false || (!graphReady && !app.rootGraphInternal)) return null;
    try {
...
function markGraphChanged(targetNode = null) {
    ...
    for (const workflow of [...new Set(workflowCandidates)]) {
        if (workflow.changeTracker?.captureCanvasState) {
...
function setWidgetValue(node, name, value, { markChanged = true } = {}) {
    const widget = getWidget(node, name);
    if (!widget || Object.is(widget.value, value)) return false;
    widget.value = value;
    widget.callback?.(value);
    if (markChanged) markGraphChanged(node);
    return true;
}
...
function loraMatchesFilter(item, filter) {
    ...
    if (filter.startsWith("folder:")) {
        const selectedFolder = filter.slice("folder:".length);
        const itemFolder = item.folder || "Root";
        return itemFolder === selectedFolder || (selectedFolder !== "Root" && itemFolder.startsWith(`${selectedFolder}/`));
    }
...
function loraTreeNodeTitle(node, isRoot = false) {
    if (isRoot) return "全部 LoRA";
    if (node?.path?.startsWith?.("folder:")) {
        const folder = node.path.slice("folder:".length);
        return folder === "Root" ? "Root" : folder;
    }
    if (node?.path?.startsWith?.("smart:")) return `分类: ${node.name}`;
    if (node?.path?.startsWith?.("sd:")) return `模型版本: ${node.name}`;
    if (node?.path?.startsWith?.("status:")) return `整理状态: ${node.name}`;
    return node?.name || "";
}
    ...
    for (const tag of visibleTags) {
        ...
        const chip = el("div", {
            ...
            ondragstart: (event) => {
                ...
                __webuiBridgeDraggedTag = {
                    prompt: tag.value,
                    local: (chip.querySelector(".webui-bridge-chip-local")?.textContent || local || "").trim(),
                    favoriteItem: false,
                    fromChip: true,
                    selectedTagValues: [...selectedTags].map((idx) => tags[idx]?.value).filter(Boolean),
                };
            },
            ondragend: () => {
                ...
                if (__webuiBridgeDraggedTag?.fromChip) __webuiBridgeDraggedTag = null;
        ...
        tools = el("div", { class: "webui-bridge-chip-tools" }, [
            ...
            tool(favorite ? "★" : "☆", favorite ? "取消该 Prompt 的全部收藏" : "加入收藏", async () => {
                ...
                if (!favorite && !name) {
                    try {
                        const locals = await translateTagsToLocal([tag.value]);
                        name = locals[0] || "";
                    } catch {}
                }
...
function promptTextWithAppendedValues(currentValue, values) {
    const additions = (Array.isArray(values) ? values : [values])
        .map((value) => String(value || "").trim())
        .filter(Boolean);
    const source = String(currentValue || "");
    if (!additions.length) return source;
    if (!source.trim()) return additions.join(EXTRA_SEPARATOR);
    const endsWithNewline = /\r?\n[ \t]*$/.test(source);
    const current = endsWithNewline
        ? source.replace(/[ \t]+$/, "")
        : source.trimEnd().replace(/[,，、]+$/, "").trimEnd();
    const separator = current && !endsWithNewline ? EXTRA_SEPARATOR : "";
    return `${current}${separator}${additions.join(EXTRA_SEPARATOR)}`;
}

function addPromptArea(textarea, text) {
    if (!text) return false;
    if (promptHasExactTag(textarea.value, text)) return false;
    setTextareaValue(textarea, promptTextWithAppendedValues(textarea.value, text));
    return true;
}
...
function bridgeTimeoutError(message) {
    ...
    error.__webuiBridgeTimeout = true;
    return error;
}

function bridgeAbortError(message = "请求已取消") {
    const error = new Error(message);
    error.name = "AbortError";
    return error;
}

async function fetchBridgeApiWithTimeout(url, options = {}, timeoutMs = 0, timeoutMessage = "请求超时，请稍后重试") {
    const externalSignal = options.signal;
    if (externalSignal?.aborted) throw bridgeAbortError();
    ...
    try {
        return await api.fetchApi(url, controller ? { ...options, signal: controller.signal } : options);
    } catch (error) {
        if (timedOut) throw bridgeTimeoutError(timeoutMessage);
        if (externalSignal?.aborted) throw bridgeAbortError();
        throw error;
    } finally {
        if (timer) {
...
async function loadBridgeData(options = {}) {
    ...
    const loadLoras = async () => {
        if (loraDetail === "basic") {
            const data = await fetchJsonWithTimeout("/webui_prompt_bridge/loras?detail=basic", fetchOptions, loraTimeoutMs, "LoRA 列表读取超时，已保留原列表");
            return { ...data, detail: "basic" };
        }
        try {
            const data = await fetchJsonWithTimeout("/webui_prompt_bridge/loras", fetchOptions, loraTimeoutMs, "LoRA 元数据读取超时，已切换到基础列表");
            return { ...data, detail: "full" };
        } catch (error) {
            console.warn("[WebUI Prompt Bridge] Full LoRA metadata load failed; falling back to basic list", error);
            const data = await fetchJsonWithTimeout("/webui_prompt_bridge/loras?detail=basic", fetchOptions, 8000, "LoRA 基础列表读取超时，已保留原列表");
            return { ...data, detail: "basic" };
        }
    };
    const [lorasRes, stylesRes, promptAllInOneRes, modelsRes, webuiRes, settingsRes] = await Promise.allSettled([
        loadLoras(),
        fetchJsonWithTimeout("/webui_prompt_bridge/styles", fetchOptions, refreshTimeoutMs, "样式读取超时"),
        fetchJsonWithTimeout("/webui_prompt_bridge/prompt_all_in_one?lang=zh_CN", fetchOptions, refreshTimeoutMs, "Prompt All in One 数据读取超时"),
        fetchJsonWithTimeout("/webui_prompt_bridge/models", fetchOptions, refreshTimeoutMs, "模型列表读取超时"),
        fetchJsonWithTimeout("/webui_prompt_bridge/webui_integration", fetchOptions, WEBUI_STATUS_TIMEOUT_MS, "WebUI 接入状态读取超时"),
        fetchJsonWithTimeout("/webui_prompt_bridge/settings", fetchOptions, refreshTimeoutMs, "设置读取超时"),
    ]);
    ...
    return {
        ...
        promptLibrary: settingsRes.status === "fulfilled" ? settingsRes.value.prompt_library || null : null,
...
function installWorkflowLoadSanitizer() {
    ...
    const installGraphConfigurePatch = () => {
        ...
        prototype.configure = function (graphData, ...args) {
            sanitizeComfyCorePackageMetadataInWorkflowData(graphData);
            const result = originalConfigure.call(this, graphData, ...args);
            scheduleBridgePanelLayoutRepairForGraph("LGraph.configure");
            return result;
        };
...
async function fetchWebUIIntegration(options = {}) {
    const response = await fetchBridgeApiWithTimeout(
        "/webui_prompt_bridge/webui_integration",
        { cache: "no-store", signal: options.signal },
        options.timeoutMs ?? WEBUI_STATUS_TIMEOUT_MS,
        "读取 WebUI 接入状态超时；如果电脑有断开的移动盘/网盘，请先断开或手动填写 WebUI 根目录后重试",
    );
    if (!response.ok) {
...
async function connectWebUIRoot(webuiRoot, autoDetect = false, options = {}) {
    const response = await fetchBridgeApiWithTimeout(
        ...
        options.timeoutMs ?? WEBUI_CONNECT_TIMEOUT_MS,
        "WebUI 接入超时；已停止等待，请确认根目录能访问后重新点击。8/9 通常只是缺少一个可选目录，不代表接入失败",
    );
...
async function clearCustomTags() {
    const response = await api.fetchApi("/webui_prompt_bridge/custom_tags?all=1", {
        method: "DELETE",
    });
    return bridgeJson(response, "清空自定义词库失败");
}
...
function installAutocomplete(input, options) {
    ...
    const render = (items) => {
        ...
        activeIndex = 0;
        popup.innerHTML = "";
        if (options.shouldShowPopup?.() === false) {
            popup.classList.remove("visible");
            return;
        }
        if (!lastItems.length) {
...
let promptOnlyLibraryDataPromise = null;
let promptOnlyLoraDataPromise = null;

function refreshPromptOnlyLibraries() {
    promptOnlyLibraryDataPromise = null;
    promptOnlyLoraDataPromise = null;
    for (const graphNode of getGraphNodes()) {
        graphNode?.__webuiBridgeRefreshPromptOnlyLibrary?.();
    }
}

function loadPromptOnlyLoras() {
    if (!promptOnlyLoraDataPromise) {
        promptOnlyLoraDataPromise = fetchJsonWithTimeout(
            "/webui_prompt_bridge/loras?detail=basic",
            { cache: "no-store" },
            10000,
            "LoRA 列表读取超时",
        ).then((data) => data.loras || []).catch((error) => {
            promptOnlyLoraDataPromise = null;
            throw error;
        });
    }
    return promptOnlyLoraDataPromise;
}

function showPromptOnlyLoraPicker(title, textarea, sync, updateHelp) {
    const mask = el("div", { class: "webui-bridge-config-mask" });
    const close = () => mask.remove();
    const search = el("input", {
        class: "webui-bridge-config-input",
        placeholder: "搜索 LoRA 名称、目录或分类…",
        spellcheck: "false",
    });
    const weight = el("input", {
        class: "webui-bridge-config-input webui-bridge-prompt-only-lora-weight",
        type: "number",
        min: "-10",
        max: "10",
        step: "0.05",
        value: "1",
        title: "插入提示词时使用的 LoRA 权重",
    });
    const categoryFilter = el("select", {
        class: "webui-bridge-config-input webui-bridge-prompt-only-lora-filter",
        title: "使用与主节点相同的 LoRA 分类筛选",
    });
    const status = el("div", { class: "webui-bridge-config-status" }, "正在读取 LoRA 列表…");
    const grid = el("div", { class: "webui-bridge-prompt-only-lora-grid" });
    let loras = [];

    const renderCategoryFilters = () => {
        const tree = buildLoraFolderTree(loras);
        const selectedFilter = categoryFilter.value || "__all";
        categoryFilter.replaceChildren(el("option", { value: "__all" }, `全部 LoRA (${loras.length})`));
        const appendGroup = (path, label) => {
            const root = tree.children.get(path);
            ...
            const appendChildren = (parent, depth = 0) => {
                ...
                for (const child of children) {
                    ...
                    group.append(el("option", { value: child.path }, `${prefix}${child.name} (${child.count})`));
                    if (child.children.size) appendChildren(child, depth + 1);
                }
            };
            appendChildren(root);
            categoryFilter.append(group);
        };
        appendGroup("group:smart", "分类");
        appendGroup("group:folders", "文件夹");
        appendGroup("group:sd", "模型版本");
        appendGroup("group:status", "整理状态");
        categoryFilter.value = [...categoryFilter.options].some((option) => option.value === selectedFilter)
            ? selectedFilter
            : "__all";
    };

    const formatWeight = () => {
        const value = clampNumber(Number(weight.value), -10, 10);
        return Number.isFinite(value) ? String(Math.round(value * 100) / 100) : "1";
    };
    const addLora = async (item, card) => {
        const name = item.alias || item.base_name || item.name;
        if (!name) return;
        card.classList.add("adding");
        status.textContent = `正在添加 ${loraDisplayName(item)}…`;
        try {
            const prompt = `<lora:${name}:${formatWeight()}>`;
            await updatePromptAreaWithLoraKeywords(
                textarea,
                prompt,
                false,
                sync,
                (message) => updateHelp?.(message),
                { toggle: false },
            );
            status.textContent = `已添加：${loraDisplayName(item)}（权重 ${formatWeight()}）`;
            card.classList.add("added");
            window.setTimeout(() => card.classList.remove("added"), 900);
        } catch (error) {
    ...
    const render = () => {
        ...
        const activeFilter = categoryFilter.value || "__all";
        const matched = loras.filter((item) => (
            loraMatchesFilter(item, activeFilter) && loraMatchesQuery(item, query)
        ));
        ...
        grid.replaceChildren(...visible.map((item) => {
            ...
            const card = el("button", {
                type: "button",
                class: "webui-bridge-prompt-only-lora-card",
                title: `${item.name || displayName}\n点击插入 LoRA 标签`,
                onclick: () => addLora(item, card),
            }, [
                item.thumbnail
                    ? el("img", {
                        src: item.thumbnail,
                        alt: "",
                        loading: "lazy",
                        onerror: (event) => event.currentTarget.replaceWith(el("span", { class: "webui-bridge-prompt-only-lora-fallback" }, "LoRA")),
                    })
                    : el("span", { class: "webui-bridge-prompt-only-lora-fallback" }, "LoRA"),
                el("span", { class: "webui-bridge-prompt-only-lora-name" }, displayName),
                el("span", { class: "webui-bridge-prompt-only-lora-folder" }, loraFolderLabel(item.folder)),
            ]);
            return card;
        }));
        status.textContent = visible.length
            ? `显示 ${visible.length} / ${matched.length} 个匹配 LoRA（全库 ${loras.length}），点击卡片即可加入提示词`
            : "没有匹配的 LoRA";
    };
    ...
    mask.append(el("div", { class: "webui-bridge-config-panel webui-bridge-prompt-only-lora-picker" }, [
        el("div", { class: "webui-bridge-config-head" }, [
            el("span", {}, `${title} · 添加 LoRA`),
            el("button", { type: "button", onclick: close }, "×"),
        ]),
        el("div", { class: "webui-bridge-config-body" }, [
            el("div", { class: "webui-bridge-config-note" }, "点击卡片会加入 LoRA 标签。把基础模型的 model、clip 接到本节点，再把本节点输出的 model、clip 接到后续工作流，LoRA 就会像主节点一样真正加载；只使用 positive_text 输出时仅保留标签文本。"),
            el("div", { class: "webui-bridge-prompt-only-lora-toolbar" }, [
                search,
                el("label", { class: "webui-bridge-prompt-only-lora-filter-control" }, [
                    el("span", {}, "分类"),
                    categoryFilter,
                ]),
                el("label", { class: "webui-bridge-prompt-only-lora-weight-control" }, [
                    el("span", {}, "权重"),
                    weight,
                ]),
            ]),
    ...
    document.body.append(mask);
    search.focus();
    loadPromptOnlyLoras().then((items) => {
        loras = items;
        renderCategoryFilters();
        render();
    }).catch((error) => {
        status.textContent = `LoRA 列表读取失败：${error?.message || error}`;
        grid.replaceChildren();
    });
}

function loadPromptOnlyLibraryData() {
    if (!promptOnlyLibraryDataPromise) {
        const fetchOptions = { cache: "no-store" };
        promptOnlyLibraryDataPromise = Promise.allSettled([
            fetchJsonWithTimeout(
                "/webui_prompt_bridge/prompt_all_in_one?lang=zh_CN",
                fetchOptions,
                WEBUI_REFRESH_TIMEOUT_MS,
                "提示词库读取超时",
            ),
            fetchJsonWithTimeout(
                "/webui_prompt_bridge/settings",
                fetchOptions,
                WEBUI_REFRESH_TIMEOUT_MS,
                "提示词设置读取超时",
            ),
        ]).then(([promptResult, settingsResult]) => {
            if (promptResult.status !== "fulfilled") throw promptResult.reason;
            return {
                promptAllInOne: promptResult.value || { group_tags: [], favorites: {} },
                settings: normalizeBridgeSettings(
                    settingsResult.status === "fulfilled" ? settingsResult.value?.settings : null,
                ),
                loras: [],
                models: { embeddings: [] },
            };
    ...
    return promptOnlyLibraryDataPromise;
}

function schedulePromptOnlyPanelInstall(node, config, configuredData = null) {
    for (const delay of [0, 32, 120, 360]) {
        window.setTimeout(() => installPromptOnlyPanel(node, config, configuredData), delay);
    }
}

function installPromptOnlyPanel(node, config, configuredData = null) {
    const promptWidget = getWidget(node, config.widgetName);
    if (!promptWidget) return;

    if (node.__webuiBridgePromptOnlyPanel) {
        const textarea = node.__webuiBridgePromptOnlyTextarea;
        if (textarea && document.activeElement !== textarea) {
            textarea.value = String(promptWidget.value || "");
            node.__webuiBridgeRenderPromptOnlyChunks?.();
        }
        const configuredSize = Array.isArray(configuredData?.size)
            ? configuredData.size.map(Number)
            : null;
        if (
            configuredSize?.length >= 2
            && Number.isFinite(configuredSize[0])
            && Number.isFinite(configuredSize[1])
        ) {
            const restoredSize = [
                Math.max(340, configuredSize[0]),
                Math.max(260, configuredSize[1]),
            ];
            node.__webuiBridgePromptOnlyConfiguredSize = [...restoredSize];
            node.size = [...restoredSize];
            node.setSize?.([...restoredSize]);
        }
        const configuredExpanded = configuredData?.properties?.webui_prompt_library_expanded;
        if (typeof configuredExpanded === "boolean") {
            node.__webuiBridgeSetPromptOnlyLibraryExpanded?.(configuredExpanded, { load: configuredExpanded });
        }
        node.__webuiBridgeApplyPromptOnlySize?.(node.size);
        return;
    }

    hideWidget(promptWidget);
    const textarea = el("textarea", {
        class: "webui-bridge-prompt-only-textarea",
        placeholder: config.placeholder,
        spellcheck: "false",
    });
    textarea.value = String(promptWidget.value || "");
    const chunkStorageKey = `webui-bridge-prompt-only-chunks-${config.kind}`;
    const autoTranslateStorageKey = `webui-bridge-aio-auto-translate-${config.kind}`;
    const shortcutStorageKey = `webui-bridge-prompt-only-translate-shortcut-${config.kind}`;
    let chunkEnabled = readLocalBoolean(chunkStorageKey, true);
    let translateShortcut = readLocalString(shortcutStorageKey, "ctrl_enter");
    if (!["ctrl_enter", "alt_t", "off"].includes(translateShortcut)) translateShortcut = "ctrl_enter";
    let promptOnlyState = null;
    const chips = el("div", { class: "webui-bridge-prompt-only-chips" });
    const chunkInput = el("input", {
        type: "text",
        class: "webui-bridge-prompt-only-chunk-input",
        placeholder: "输入提示词，逗号或回车生成分块…",
        spellcheck: "false",
    });
    ...
    const renderChunks = () => {
        textarea.hidden = chunkEnabled;
        chips.hidden = !chunkEnabled;
        if (chunkEnabled) {
            renderPromptOnlyChunks(chips, textarea, promptOnlyState, {
                input: chunkInput,
                activeIndex: activeChunkIndex,
                onSelect: (index) => {
                    activeChunkIndex = index;
                    [...chips.querySelectorAll(".webui-bridge-prompt-only-chip")]
                        .forEach((chip, chipIndex) => chip.classList.toggle("active", chipIndex === index));
                },
                onDelete: (index) => {
                    if (!removePromptTagAt(textarea, index)) return;
                    activeChunkIndex = Math.min(activeChunkIndex, splitPromptTags(textarea.value).length - 1);
                    sync();
                    chunkInput.focus();
                },
                onEdit: (index) => {
                    const tag = splitPromptTags(textarea.value)[index];
                    if (!tag || !removePromptTagAt(textarea, index)) return;
                    chunkInput.value = tag.value;
                    activeChunkIndex = -1;
                    sync();
                    chunkInput.focus();
                    chunkInput.select();
                ...
                onToggleSeparator: (index) => {
                    const tag = splitPromptTags(textarea.value)[index];
                    if (!tag) return;
                    const changed = promptSeparatorMarker(tag.separatorAfter)
                        ? removePromptTagSeparatorAt(textarea, index)
                        : setPromptTagSeparatorAt(textarea, index, ",\n");
                    if (changed) sync();
                },
            });
        }
    ...
    let promptOnlyInputTimer = 0;
    let promptOnlyGraphDirty = false;
    const sync = ({ defer = false } = {}) => {
        const next = textarea.value;
        if (promptWidget.value !== next) {
            promptWidget.value = next;
            promptWidget.callback?.(next);
            promptOnlyGraphDirty = true;
        }
        if (defer) {
            window.clearTimeout?.(promptOnlyInputTimer);
            promptOnlyInputTimer = window.setTimeout?.(() => {
                promptOnlyInputTimer = 0;
                if (promptOnlyGraphDirty) markGraphChanged(node);
                promptOnlyGraphDirty = false;
                renderChunks();
            }, 140) || 0;
            return;
        }
    ...
    textarea.addEventListener("input", (event) => {
        if (!textarea.__webuiBridgeSettingValue) sync({ defer: Boolean(event.isTrusted) });
    });
    const appendChunkEntries = async (values, options = {}) => {
        const rawValues = (values || []).map((value) => String(value || "").trim()).filter(Boolean);
        if (!rawValues.length) return;
        const translatedValues = [];
        const shouldTranslate = options.translate !== false && readLocalBoolean(autoTranslateStorageKey, true);
        for (const raw of rawValues) {
            if (shouldTranslate && /[\u3400-\u9fff]/.test(raw)) {
                const resolved = await resolveKeywordInput(raw, true);
                translatedValues.push(...resolved.map((item) => item.prompt).filter(Boolean));
            } else {
                translatedValues.push(raw);
            }
        }
        if (!translatedValues.length) return;
        setTextareaValue(textarea, promptTextWithAppendedValues(textarea.value, translatedValues));
        activeChunkIndex = splitPromptTags(textarea.value).length - 1;
        sync();
    };
    const queueChunkEntries = (values, options = {}) => {
        chunkCommitQueue = chunkCommitQueue
            .then(() => appendChunkEntries(values, options))
            .catch((error) => updateHelp(`添加失败：${error?.message || error}`));
        return chunkCommitQueue;
    };
    chunkInput.addEventListener("input", () => {
        const raw = chunkInput.value;
        if (!/[,，、\n\r]/.test(raw)) return;
        const parts = raw.split(/[,，、\n\r]+/);
        const endsWithSeparator = /[,，、\n\r]\s*$/.test(raw);
        const remainder = endsWithSeparator ? "" : (parts.pop() || "");
        const completed = parts.map((value) => value.trim()).filter(Boolean);
        chunkInput.value = remainder;
        if (completed.length) queueChunkEntries(completed);
    });
    chips.addEventListener("click", (event) => {
        if (event.target === chips) chunkInput.focus();
    });
    const translateCurrentPrompt = async () => {
        const value = textarea.value.trim();
        if (!value) {
            updateHelp("提示词为空，无需翻译");
            return;
        }
        if (!/[\u3400-\u9fff]/.test(value)) {
            updateHelp("没有检测到需要翻译的中文");
            return;
        }
        updateHelp("正在翻译整段提示词…");
        textarea.classList.add("webui-bridge-input-busy");
        try {
            const translated = await translatePromptAllInOne(value, "english");
            setTextareaValue(textarea, translated.prompt || value);
            sync();
            updateHelp(translated.matched ? `已翻译 ${translated.matched} 个关键词` : "本地词库未匹配，已保留原文");
        } catch (error) {
            updateHelp(`翻译失败：${error?.message || error}`);
        } finally {
            textarea.classList.remove("webui-bridge-input-busy");
            window.setTimeout(() => updateHelp(), 2400);
        }
    };
    textarea.addEventListener("keydown", (event) => {
        const translatePressed = translateShortcut === "ctrl_enter"
            ? (event.ctrlKey || event.metaKey) && event.key === "Enter"
            : translateShortcut === "alt_t" && event.altKey && String(event.key).toLowerCase() === "t";
        if (translatePressed) {
            event.preventDefault();
            event.stopPropagation();
    ...
    installAutocomplete(textarea, {
        limit: 8,
        showOnFocus: false,
        getQuery: () => promptTokenAtCursor(textarea).value,
        onPick: (suggestion) => {
            replacePromptToken(textarea, suggestion);
            sync();
        },
    });
    installAutocomplete(chunkInput, {
        limit: 8,
        showOnFocus: false,
        compact: true,
        getQuery: () => chunkInput.value,
        onPick: async (suggestion) => {
            const prompt = suggestion.text || suggestion.prompt || "";
            if (!prompt) return;
            chunkInput.value = "";
            await queueChunkEntries([prompt], { translate: false });
            chunkInput.focus();
        },
    });
    chunkInput.addEventListener("keydown", (event) => {
        const popupVisible = chunkInput.__webuiBridgeAutocompletePopup?.classList.contains("visible");
        const translatePressed = translateShortcut === "ctrl_enter"
            ? (event.ctrlKey || event.metaKey) && event.key === "Enter"
            : translateShortcut === "alt_t" && event.altKey && String(event.key).toLowerCase() === "t";
        if (translatePressed) {
        ...
        if ((event.key === "Enter" || event.key === "Tab") && !popupVisible) {
            const value = chunkInput.value.trim();
            if (!value) return;
            event.preventDefault();
            event.stopPropagation();
            chunkInput.value = "";
            queueChunkEntries([value]).then(() => chunkInput.focus());
            return;
        }
        if (event.key === "Backspace" && !chunkInput.value && !popupVisible) {
            const tags = splitPromptTags(textarea.value);
            if (!tags.length) return;
            event.preventDefault();
            const index = activeChunkIndex >= 0 ? activeChunkIndex : tags.length - 1;
            if (removePromptTagAt(textarea, index)) {
                activeChunkIndex = Math.min(index - 1, splitPromptTags(textarea.value).length - 1);
                sync();
            }
            return;
        }
        if ((event.ctrlKey || event.metaKey) && (event.key === "ArrowUp" || event.key === "ArrowDown")) {
            const tag = splitPromptTags(textarea.value)[activeChunkIndex];
            if (!tag) return;
            event.preventDefault();
            if (replacePromptTagAt(textarea, activeChunkIndex, changeTagNumericWeight(tag.value, event.key === "ArrowUp" ? 0.1 : -0.1))) sync();
            return;
        }
    ...
    updateHelp();
    renderChunks();
    loadPromptOnlyLibraryData().then((state) => {
        promptOnlyState = state;
        renderChunks();
    }).catch(() => {});

    const libraryToggle = el("button", {
        type: "button",
        class: "webui-bridge-prompt-only-library-toggle",
        title: "展开分类提示词库；首次展开时加载数据",
    });
    const settingsButton = el("button", {
        type: "button",
        class: "webui-bridge-prompt-only-settings",
        title: `${config.label}设置`,
        onclick: () => showPromptManagementSettings(
            config.label,
            autoTranslateStorageKey,
            node.__webuiBridgePromptOnlyLibraryPanel?.__webuiBridgeAutoTranslateInput || null,
            {
                smallNode: true,
                chunkEnabled,
                shortcutValue: translateShortcut,
                onChunkChange: (enabled) => {
                    chunkEnabled = Boolean(enabled);
                    writeLocalBoolean(chunkStorageKey, chunkEnabled);
                    renderChunks();
                    if (chunkEnabled && Number(node.size?.[1] || 0) < 340) {
                        node.setSize?.([Math.max(340, Number(node.size?.[0] || 0)), 360]);
                    }
                    node.__webuiBridgeApplyPromptOnlySize?.(node.size);
                },
                onShortcutChange: (value) => {
                    translateShortcut = ["ctrl_enter", "alt_t", "off"].includes(value) ? value : "ctrl_enter";
                    writeLocalString(shortcutStorageKey, translateShortcut);
                    updateHelp();
                },
            },
        ),
    }, "⚙");
    const loraButton = config.kind === "positive" ? el("button", {
        type: "button",
        class: "webui-bridge-prompt-only-lora-button",
        title: "打开 LoRA 列表并加入当前提示词",
        onclick: () => showPromptOnlyLoraPicker(config.label, textarea, sync, updateHelp),
    }, "LoRA +") : null;
    const libraryShell = el("div", { class: "webui-bridge-prompt-only-library", hidden: "hidden" });
    let libraryLoaded = false;
    let libraryLoading = false;
    let libraryLoadToken = 0;

    const panel = el("div", { class: "webui-bridge-prompt-only-panel" }, [
        el("div", { class: "webui-bridge-prompt-only-label" }, [
            el("strong", {}, config.label),
            el("div", { class: "webui-bridge-prompt-only-head-actions" }, [
                el("span", {}, "Tag 自动补全"),
                settingsButton,
                loraButton,
                libraryToggle,
    ...
    const loadLibrary = async () => {
        ...
        libraryLoading = true;
        libraryShell.replaceChildren(el("div", { class: "webui-bridge-prompt-only-loading" }, "正在加载提示词库…"));
        try {
            const state = await loadPromptOnlyLibraryData();
            if (!libraryShell.isConnected || loadToken !== libraryLoadToken) return;
            const libraryPanel = createPromptAllInOnePanel(
                config.kind,
                config.kind === "negative" ? "反向词" : "提示词",
                textarea,
                state,
                sync,
            );
            libraryPanel.classList.add("webui-bridge-prompt-only-aio");
            node.__webuiBridgePromptOnlyLibraryPanel = libraryPanel;
            libraryPanel.style.removeProperty("height");
            libraryPanel.style.removeProperty("min-height");
            libraryPanel.style.removeProperty("max-height");
            libraryShell.replaceChildren(libraryPanel);
            libraryLoaded = true;
            node.__webuiBridgeApplyPromptOnlySize?.(node.size);
        } catch (error) {
            if (loadToken !== libraryLoadToken) return;
            libraryShell.replaceChildren(el("div", { class: "webui-bridge-prompt-only-loading error" }, `提示词库加载失败：${error?.message || error}`));
        } finally {
            if (loadToken === libraryLoadToken) libraryLoading = false;
        }
    };
    const setLibraryExpanded = (expanded, options = {}) => {
        const next = Boolean(expanded);
        panel.classList.toggle("library-expanded", next);
        libraryShell.hidden = !next;
        libraryToggle.textContent = next ? "收起词库 ▴" : "展开词库 ▾";
        libraryToggle.classList.toggle("active", next);
        node.properties = node.properties && typeof node.properties === "object" ? node.properties : {};
        node.properties.webui_prompt_library_expanded = next;
        if (next && options.load !== false) loadLibrary();
        node.__webuiBridgeApplyPromptOnlySize?.(node.size);
        if (!options.silent) markGraphChanged(node);
    };
    libraryToggle.addEventListener("click", () => {
        const expanded = !panel.classList.contains("library-expanded");
        node.properties = node.properties && typeof node.properties === "object" ? node.properties : {};
        if (expanded) {
            const collapsedSize = [
                Math.max(340, Number(node.size?.[0] || 420)),
                Math.max(260, Number(node.size?.[1] || 260)),
            ];
            node.__webuiBridgePromptOnlyCollapsedSize = [...collapsedSize];
            node.properties.webui_prompt_library_collapsed_size = [...collapsedSize];
            if (collapsedSize[1] < 560) {
                node.setSize?.([Math.max(520, collapsedSize[0]), 720]);
            }
            setLibraryExpanded(true, { silent: true });
        } else {
            setLibraryExpanded(false, { silent: true });
            const savedCollapsedSize = node.__webuiBridgePromptOnlyCollapsedSize
                || node.properties.webui_prompt_library_collapsed_size;
            if (
                Array.isArray(savedCollapsedSize)
                && savedCollapsedSize.length >= 2
                && Number.isFinite(Number(savedCollapsedSize[0]))
                && Number.isFinite(Number(savedCollapsedSize[1]))
            ) {
                const restoredSize = [
                    Math.max(340, Number(savedCollapsedSize[0])),
                    Math.max(260, Number(savedCollapsedSize[1])),
                ];
                node.size = [...restoredSize];
                node.setSize?.([...restoredSize]);
                node.__webuiBridgeApplyPromptOnlySize?.(restoredSize);
            }
            delete node.__webuiBridgePromptOnlyCollapsedSize;
            delete node.properties.webui_prompt_library_collapsed_size;
        }
        markGraphChanged(node);
    });

    const domWidget = node.addDOMWidget("webui_bridge_prompt_only", "webui_bridge_prompt_only", panel, {
        serialize: false,
        hideOnZoom: false,
        getMinHeight: () => 190,
        getMaxHeight: () => 2400,
    ...
    installWidgetSerializationFallback(domWidget, () => null);
    const applySize = (size = node.size) => {
        const nodeWidth = Math.max(340, Number(size?.[0] || 420));
        const nodeHeight = Math.max(260, Number(size?.[1] || 260));
        const widgetWidth = Math.max(300, nodeWidth - 40);
        const widgetHeight = Math.max(190, nodeHeight - 70);
        panel.style.width = `${widgetWidth}px`;
        panel.style.height = `${widgetHeight}px`;
        syncBridgeDomWidgetWrapper(panel, widgetWidth, widgetHeight, { minWidth: 300, minHeight: 190 });
        return [widgetWidth, widgetHeight];
    };
    domWidget.computeSize = () => [
        300,
        panel.classList.contains("library-expanded") ? (chunkEnabled ? 500 : 430) : (chunkEnabled ? 290 : 190),
    ];
    node.__webuiBridgePromptOnlyPanel = panel;
    node.__webuiBridgePromptOnlyTextarea = textarea;
    node.__webuiBridgeSetPromptOnlyLibraryExpanded = setLibraryExpanded;
    node.__webuiBridgeApplyPromptOnlySize = applySize;
    node.__webuiBridgeRenderPromptOnlyChunks = renderChunks;
    node.__webuiBridgeRefreshPromptOnlyLibrary = () => {
        libraryLoadToken += 1;
        libraryLoaded = false;
        libraryLoading = false;
        node.__webuiBridgePromptOnlyLibraryPanel = null;
        libraryShell.replaceChildren();
        loadPromptOnlyLibraryData().then((state) => {
            promptOnlyState = state;
            renderChunks();
        }).catch(() => {});
        if (panel.classList.contains("library-expanded")) loadLibrary();
    };
    node.color = config.color;
    node.bgcolor = config.bgcolor;
    node.resizable = true;
    const configuredSize = Array.isArray(configuredData?.size)
        ? configuredData.size.map(Number)
        : null;
    const hasValidConfiguredSize = configuredSize?.length >= 2
        && Number.isFinite(configuredSize[0])
        && Number.isFinite(configuredSize[1]);
    const baseInitialSize = hasValidConfiguredSize ? configuredSize : [420, chunkEnabled ? 360 : 260];
    const initialSize = [
        Math.max(340, Number(baseInitialSize[0] || 0)),
        Math.max(260, Number(baseInitialSize[1] || 0)),
    ];
    if (hasValidConfiguredSize) node.__webuiBridgePromptOnlyConfiguredSize = [...initialSize];
    node.size = [...initialSize];
    node.setSize?.([...initialSize]);
    applySize(initialSize);
    chainCallback(node, "onResize", function (size) {
        applySize(size || this.size);
    });
    const configuredExpanded = configuredData?.properties?.webui_prompt_library_expanded;
...
function createPromptRow(label, value, placeholder, onFocus, onInput, options = {}) {
    ...
    textChipGrip = createHeightSplitGrip(textarea, chips, textareaHeightKey, chipHeightKey, {
        ...
        onDragStart: () => markManualResize({ phase: "start" }),
        onDragMove: () => markManualResize({ phase: "move" }),
        onDragEnd: () => markManualResize({ phase: "end", commit: true }),
    });
    chipBottomGrip = createHeightResizeGrip(chips, chipHeightKey, {
        min: PROMPT_CHIPS_MIN_HEIGHT,
        max: 520,
        title: "拖动调整已输入 tag 区底边高度，双击恢复",
        onDragStart: () => markManualResize({ phase: "start" }),
        onDragMove: () => markManualResize({ phase: "move" }),
        onDragEnd: () => markManualResize({ phase: "end", commit: true }),
    });
...
function showPromptManagementSettings(title, translateStorageKey, autoTranslate = null, options = {}) {
    const mask = el("div", { class: "webui-bridge-config-mask" });
    const close = () => mask.remove();
    const autoTranslateSetting = el("input", { type: "checkbox" });
    autoTranslateSetting.checked = autoTranslate
        ? Boolean(autoTranslate.checked)
        : readLocalBoolean(translateStorageKey, true);
    autoTranslateSetting.addEventListener("change", () => {
        if (autoTranslate) autoTranslate.checked = autoTranslateSetting.checked;
        writeLocalBoolean(translateStorageKey, autoTranslateSetting.checked);
    });
    const chunkSetting = options.smallNode ? el("input", { type: "checkbox" }) : null;
    if (chunkSetting) {
        chunkSetting.checked = options.chunkEnabled !== false;
        chunkSetting.addEventListener("change", () => options.onChunkChange?.(chunkSetting.checked));
    }
    const shortcutSetting = options.smallNode ? el("select", { class: "webui-bridge-config-input" }, [
        el("option", { value: "ctrl_enter" }, "Ctrl+Enter（推荐）"),
        el("option", { value: "alt_t" }, "Alt+T"),
        el("option", { value: "off" }, "关闭快捷键"),
    ]) : null;
    if (shortcutSetting) {
        shortcutSetting.value = ["ctrl_enter", "alt_t", "off"].includes(options.shortcutValue)
            ? options.shortcutValue
            : "ctrl_enter";
        shortcutSetting.addEventListener("change", () => options.onShortcutChange?.(shortcutSetting.value));
    }
    const notes = options.smallNode
        ? [
            "分块模式会把整个编辑区变成 Tag 编辑器：输入后按逗号、回车或 Tab 立即生成卡片；关闭后恢复普通文本框。",
            "Ctrl+Enter 默认用于翻译整个提示词框；Alt+T 适合习惯字母快捷键的用户。两者都不会和 Ctrl+↑↓ 调权重、Alt+←→ 移动 Tag 冲突。",
            "“回车添加时翻译中文”同时作用于分块编辑器和展开词库后的关键词添加；关闭后按原文生成 Tag。",
            "“未匹配本地关键词组，保留原文”表示本地词库没有对应映射，并非生成错误；可以换成更短的关键词或在主节点设置中补充词库。",
        ]
        : [
    ...
    mask.append(el("div", { class: "webui-bridge-config-panel webui-bridge-prompt-settings" }, [
        ...
        el("div", { class: "webui-bridge-config-body" }, [
            ...
            ...notes.map((note) => el("div", { class: "webui-bridge-config-note" }, note)),
        ]),
        el("div", { class: "webui-bridge-config-actions" }, [
            el("button", { type: "button", class: "primary", onclick: close }, "完成"),
        ]),
...
function renderPromptOnlyChunks(chips, textarea, state = null, options = {}) {
    if (!chips) return;
    const localMap = state ? buildLocalTagMap(state) : new Map();
    ...
    for (let index = 0; index < tags.length; index += 1) {
        ...
        const lora = parseLoraTag(tag.value);
        const local = lora ? "LoRA" : localMap.get(tag.value.toLowerCase()) || "";
        const chip = el("div", {
            ...
            ondragstart: (event) => {
                ...
                chips.__webuiBridgeDraggingIndex = index;
                chip.classList.add("dragging");
                chips.classList.add("drag-active");
                event.dataTransfer.effectAllowed = "move";
            ...
            el("button", {
                type: "button",
                class: "webui-bridge-prompt-only-chip-content",
                draggable: "false",
                title: "单击选择，双击编辑",
            ...
            el("button", {
                type: "button",
                class: `webui-bridge-prompt-only-chip-separator${promptSeparatorMarker(tag.separatorAfter) ? " active" : ""}`,
                draggable: "false",
                title: promptSeparatorMarker(tag.separatorAfter) ? "取消这个 Tag 后的换行" : "在这个 Tag 后换行",
                onclick: (event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    options.onToggleSeparator?.(index);
                },
            }, "↵"),
        ].filter(Boolean));
        items.push(chip);
        const separatorMarker = promptSeparatorMarker(tag.separatorAfter);
        if (separatorMarker) {
            items.push(el("button", {
                type: "button",
                class: "webui-bridge-prompt-only-separator-marker",
                title: `${separatorMarker.title}；点击取消`,
                onclick: () => options.onToggleSeparator?.(index),
            }, separatorMarker.text));
        }
    }
    if (options.input) items.push(options.input);
    chips.replaceChildren(...items);
    chips.classList.toggle("empty", tags.length === 0);
}
...
function __webuiBridgeHandleDragStart(event, tag) {
    ...
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", tag.id || tag.prompt);
}

function __webuiBridgeHandleDragOver(event) {
    event.preventDefault();
    event.dataTransfer.dropEffect = "move";
}
...
function createTagButton(tag, textarea, sync, rerender, isNegative = false, state = null) {
    ...
    const button = el("button", {
        ...
        "data-prompt": prompt,
        draggable: tag.draggable ? "true" : "false",
...
function createPromptAllInOnePanel(kind, title, textarea, state, sync) {
    ...
    body.addEventListener("dragover", (e) => {
        ...
        if (dragged?.favoriteItem) {
            const bodyGroupItem = groups[activeGroup];
            const bodyCat = bodyGroupItem?.categoryName || bodyGroupItem?.name?.replace(/^★ /, "");
            const cleanSubs = (groups[activeGroup]?.groups || []).filter((g) => g.type !== "wrap");
            const bodySubCat = cleanSubs[activeSubGroup]?.name || "";
            if (dragged.category === bodyCat && dragged.subCategory === bodySubCat) {
                e.preventDefault();
                e.dataTransfer.dropEffect = "move";
                return;
            }
        }
        __webuiBridgeHandleDragOver(e);
        clearTimeout(__webuiBridgeBodyDragTimer);
        body.classList.add("drop-target");
    });
    body.addEventListener("dragleave", () => {
        __webuiBridgeBodyDragTimer = setTimeout(() => {
    ...
    body.addEventListener("drop", (e) => {
        clearTimeout(__webuiBridgeBodyDragTimer);
        body.classList.remove("drop-target");
        // Skip body drop only for same-area tag reorder (not cross-category transfer)
        const dragged = __webuiBridgeDraggedTag;
        if (dragged?.favoriteItem) {
            const bodyGroupItem = groups[activeGroup];
            const bodyCat = bodyGroupItem?.categoryName || bodyGroupItem?.name?.replace(/^★ /, "");
        ...
        const curActiveGroupItem = groups[activeGroup];
        const isFav = curActiveGroupItem?.type === "favorite";
        const curCleanSubGroups = (groups[activeGroup]?.groups || []).filter((g) => g.type !== "wrap");
        const curSubGroup = curCleanSubGroups[activeSubGroup];
        if (isFav) {
            const targetCat = curActiveGroupItem?.categoryName || curActiveGroupItem?.name?.replace(/^★ /, "");
            const targetSubCat = curSubGroup?.name || "";
            handleDropOnCategory(e, targetCat, targetSubCat).catch(() => {});
        } else {
            handleDropOnCategory(e, "新增提示词", "未分类").catch(() => {});
        }
    });
    ...
    root.__webuiBridgeAutoTranslateInput = autoTranslate;
    ...
    const showPromptSettings = () => showPromptManagementSettings(title, translateStorageKey, autoTranslate);
    ...
    const sortByCustomOrder = (items, orderArray, defaultFirst = "") => {
        const orderMap = new Map(orderArray.map((name, i) => [name, i]));
        return [...items].sort((a, b) => {
            if (defaultFirst && a === defaultFirst) return -1;
            if (defaultFirst && b === defaultFirst) return 1;
    ...
    const buildGroups = () => {
        ...
        const localTagMap = buildLocalTagMap(state);

        // 将收藏项按父子分类分组
        const defaultCategory = "新增提示词";
        const defaultSubCategory = "未分类";
        const categoryMap = new Map(); // category -> Map(subCategory -> [items])
        ...
        favoriteItems.forEach((item) => {
            ...
            const subCat = item.subCategory || defaultSubCategory;
            ...
            // 优先使用用户自定义名称，其次尝试从本地翻译映射获取中文
            let local = item.name && item.name !== item.prompt ? item.name : "";
            if (!local) {
                const storedTags = item.tags || [];
                if (storedTags.length === 1) {
                    const firstTag = storedTags[0];
                    if (firstTag.localValue && firstTag.localValue !== item.prompt) {
                        local = firstTag.localValue;
                    }
                }
            }
            if (!local) {
                const singlePrompt = item.prompt.trim();
                if (!singlePrompt.includes(",") && localTranslationCache.has(singlePrompt.toLowerCase())) {
                    local = localTranslationCache.get(singlePrompt.toLowerCase());
                }
            }
            if (!local) {
                const singlePrompt = item.prompt.trim();
                if (!singlePrompt.includes(",") && localTagMap.has(singlePrompt.toLowerCase())) {
                    local = localTagMap.get(singlePrompt.toLowerCase());
                }
            }
            subMap.get(subCat).push({
                id: item.id,
                prompt: item.prompt,
                local: local || item.prompt,
        ...
        const fullSearchGroup = {
            name: "全库搜索",
            type: "fullSearch",
            groups: [{
                name: "TagComplete",
                tags: fullSearchItems,
            }],
        };
        ...
        groups = [...sourceGroups, ...favoriteGroups, extraGroup, fullSearchGroup];
    ...
    function renderTabs() {
        ...
        groups.forEach((group, index) => {
            const isFavorite = group.type === "favorite";
            const oldName = group.categoryName || group.name?.replace(/^★ /, "");
            const isDefault = oldName === "新增提示词";
            const tabBtn = el("button", {
                class: (index === activeGroup ? "active" : "") + (isFavorite ? " webui-bridge-tab-favorite" : ""),
                onclick: () => {
                    activeGroup = index;
                    activeSubGroup = 0;
                    favoritePage = 0;
            ...
            if (isFavorite) {
                ...
                tabBtn.addEventListener("dragstart", (e) => {
                    __webuiBridgeTabDrag = { catName: oldName, type: "parent" };
                ...
                tabBtn.addEventListener("dragover", (e) => {
                    if (__webuiBridgeTabDrag && __webuiBridgeTabDrag.type === "parent") {
                        ...
                        tabBtn.classList.add(after ? "webui-bridge-tab-insert-after" : "webui-bridge-tab-insert-before");
                    } else {
                        __webuiBridgeHandleDragOver(e);
                ...
                tabBtn.addEventListener("dragleave", (e) => {
                    if (!__webuiBridgeTabDrag) {
                        tabBtn.classList.remove("drop-target");
                    }
                    tabBtn.classList.remove("webui-bridge-tab-insert-before", "webui-bridge-tab-insert-after");
                });
                tabBtn.addEventListener("drop", (e) => {
                    e.stopPropagation();
                    tabBtn.classList.remove("drop-target", "webui-bridge-tab-insert-before", "webui-bridge-tab-insert-after");
                    if (__webuiBridgeTabDrag && __webuiBridgeTabDrag.type === "parent") {
                        const fromCatName = __webuiBridgeTabDrag.catName;
                        const toCatName = oldName;
                        const rect = tabBtn.getBoundingClientRect();
                        const after = e.clientX > rect.left + rect.width / 2;
                        const customCatsKey = `webui-bridge-favorite-categories-${kind}`;
                    ...
                    handleDropOnCategory(e, oldName, "未分类").catch(() => {});
                });
                let popup = null;
                const closePopup = () => {
                ...
                const showPopup = () => {
                    ...
                    const renameItem = el("div", {
                        class: "webui-bridge-tab-popup-item",
                        onclick: async (e) => {
                            e.stopPropagation();
                            closePopup();
                    ...
                    if (!isDefault) {
                        const deleteItem = el("div", {
                            class: "webui-bridge-tab-popup-item webui-bridge-tab-popup-danger",
                            onclick: async (e) => {
                                e.stopPropagation();
                                closePopup();
                                const favoriteItems = state.promptAllInOne?.favorites?.[kind] || [];
                    ...
                    popup.addEventListener("mouseleave", (e) => {
                        if (!tabBtn.contains(e.relatedTarget)) closePopup();
                    });
                    document.body.append(popup);
                };
                tabBtn.addEventListener("mouseenter", showPopup);
                tabBtn.addEventListener("mouseleave", (e) => {
                    if (!popup || !popup.contains(e.relatedTarget)) closePopup();
                });
            }
        });
        // 添加父级收藏分类按钮
        const addParentBtn = el("button", {
            class: "webui-bridge-tab-add",
            title: "添加新的收藏分类",
            onclick: async () => {
    ...
    function renderSubTabs() {
        ...
        cleanSubGroups.forEach((group, index) => {
            const label = group.name || "Tags";
            const isFavorite = activeGroupItem?.type === "favorite";
            const isDefault = label === "未分类";
            const parentCatName = activeGroupItem?.categoryName || activeGroupItem?.name?.replace(/^★ /, "");
            const btn = el("button", {
            ...
            if (isFavorite) {
                ...
                btn.addEventListener("dragstart", (e) => {
                    __webuiBridgeTabDrag = { subCatName: label, type: "child", parentCatName };
                    e.dataTransfer.effectAllowed = "move";
                ...
                btn.addEventListener("dragover", (e) => {
                    if (__webuiBridgeTabDrag && __webuiBridgeTabDrag.type === "child") {
                        ...
                        const after = e.clientX > rect.left + rect.width / 2;
                        btn.classList.add(after ? "webui-bridge-tab-insert-after" : "webui-bridge-tab-insert-before");
                    } else {
                        __webuiBridgeHandleDragOver(e);
                ...
                btn.addEventListener("dragleave", (e) => {
                    ...
                    btn.classList.remove("webui-bridge-tab-insert-before", "webui-bridge-tab-insert-after");
                });
                btn.addEventListener("drop", (e) => {
                    e.stopPropagation();
                    btn.classList.remove("drop-target", "webui-bridge-tab-insert-before", "webui-bridge-tab-insert-after");
                    if (__webuiBridgeTabDrag && __webuiBridgeTabDrag.type === "child") {
                        const fromSubCatName = __webuiBridgeTabDrag.subCatName;
                        const toSubCatName = label;
                        const rect = btn.getBoundingClientRect();
                        const after = e.clientX > rect.left + rect.width / 2;
                    ...
                    handleDropOnCategory(e, parentCatName, label).catch(() => {});
                });
                let popup = null;
                const closeSubPopup = () => {
                    if (popup) { popup.remove(); popup = null; }
                ...
                const showSubPopup = () => {
                    ...
                    popup.addEventListener("mouseleave", (e) => {
                        if (!btn.contains(e.relatedTarget)) closeSubPopup();
                    });
                    document.body.append(popup);
                };
                btn.addEventListener("mouseenter", showSubPopup);
                btn.addEventListener("mouseleave", (e) => {
                    if (!popup || !popup.contains(e.relatedTarget)) closeSubPopup();
                });
            }
            subTabs.append(btn);
        });
        // 添加子级收藏分类按钮
        if (activeGroupItem?.type === "favorite") {
            const addChildBtn = el("button", {
                class: "webui-bridge-tab-add",
                title: "在当前分类下添加子分类",
                onclick: async () => {
                    const name = prompt("输入新的子级分类名称", "");
                    if (!name) return;
    ...
    function render() {
        document.querySelectorAll(".webui-bridge-tab-popup").forEach((p) => p.remove());
        buildGroups();
        renderTabs();
        renderSubTabs();
        renderBody();
    ...
    installAutocomplete(query, {
        ...
        popupMaxWidth: 440,
        popupMaxHeight: 224,
        shouldShowPopup: () => groups[activeGroup]?.type !== "fullSearch",
        onResults: (items, searchedQuery) => {
            fullSearchQuery = String(searchedQuery || "").trim().toLowerCase();
            fullSearchItems = (items || []).map((item) => ({
                prompt: item.text || "",
                local: item.local || item.text || "",
                count: item.count || 0,
            })).filter((item) => item.prompt);
            if (groups[activeGroup]?.type === "fullSearch" && query.value.trim().toLowerCase() === fullSearchQuery) {
                renderBody();
            }
        },
        onPick: async (item) => {
...
function buildPanel(node) {
    repairBridgeNodeWidgetDefaults(node);
    const initialLayoutPreset = normalizeBridgeLayoutPreset(
        node.__webuiBridgeInitialLayoutPreset || readBridgeInitialLayoutPreset(),
    );
    node.__webuiBridgeInitialLayoutPreset = initialLayoutPreset;
    const positiveWidget = getWidget(node, "positive_prompt");
    const negativeWidget = getWidget(node, "negative_prompt");
    const clipStrengthWidget = getWidget(node, "default_clip_strength");
    ...
    const state = {
        ...
        settings: normalizeBridgeSettings({ layout_preset: initialLayoutPreset }),
        customTagCount: 0,
        promptLibrary: null,
        assets: null,
        moduleAssets: null,
    ...
    const {
        ...
        extra_height: extraHeightKey,
        extra_collapsed: extraCollapsedKey,
        sidebar_collapsed: actionCollapsedKey,
        sidebar_width: actionWidthKey,
        negative_collapsed: negativeCollapsedKey,
        positive_textarea_height: positiveTextareaHeightKey,
        negative_textarea_height: negativeTextareaHeightKey,
        positive_chip_height: positiveChipHeightKey,
        negative_chip_height: negativeChipHeightKey,
        positive_tag_height: positiveTagHeightKey,
        negative_tag_height: negativeTagHeightKey,
        lora_scroll_top: loraScrollTopKey,
    } = BRIDGE_LAYOUT_STORAGE_KEYS;
    const configuredLayoutState = normalizeBridgeLayoutState(node.__webuiBridgeConfiguredLayoutState);
    ...
    let promptInputSyncTimer = 0;
    let promptInputGraphDirty = false;
    const pendingPromptInputSides = new Set();
    const clearPendingPromptInputSync = () => {
        window.clearTimeout?.(promptInputSyncTimer);
        promptInputSyncTimer = 0;
        const graphDirty = promptInputGraphDirty;
        promptInputGraphDirty = false;
        pendingPromptInputSides.clear();
        return graphDirty;
    };
    const flushPromptInputSync = () => {
        promptInputSyncTimer = 0;
        if (promptInputGraphDirty) markGraphChanged(node);
        promptInputGraphDirty = false;
        const sides = [...pendingPromptInputSides];
        pendingPromptInputSides.clear();
        if (sides.length > 1) updateCounters("both");
        else if (sides[0]) updateCounters(sides[0]);
        node.__webuiBridgePanel?.__webuiBridgeUpdateZoomSummary?.();
    };
    const schedulePromptInputSync = (textarea) => {
        const side = textarea.__webuiBridgeKind || "both";
        if (side !== "negative") {
            promptInputGraphDirty = setWidgetValue(node, "positive_prompt", positive.textarea.value, { markChanged: false }) || promptInputGraphDirty;
        }
        if (side !== "positive") {
            promptInputGraphDirty = setWidgetValue(node, "negative_prompt", negative.textarea.value, { markChanged: false }) || promptInputGraphDirty;
        }
        pendingPromptInputSides.add(side);
        window.clearTimeout?.(promptInputSyncTimer);
        promptInputSyncTimer = window.setTimeout?.(flushPromptInputSync, 140) || 0;
    };
    const sync = (side = "both") => {
        const pendingGraphDirty = clearPendingPromptInputSync();
        let promptChanged = false;
        if (side !== "negative") promptChanged = setWidgetValue(node, "positive_prompt", positive.textarea.value) || promptChanged;
        if (side !== "positive") promptChanged = setWidgetValue(node, "negative_prompt", negative.textarea.value) || promptChanged;
        if (pendingGraphDirty && !promptChanged) markGraphChanged(node);
        if (side === "both") {
            syncRegionalWidgets?.();
    ...
    const onInput = (textarea, event = null) => {
        ...
        if (textarea.__webuiBridgeAutoTranslating) {
            ...
            if (event?.isTrusted) schedulePromptInputSync(textarea);
            else sync(textarea.__webuiBridgeKind || "both");
            return;
        }
    ...
    const positive = createPromptRow(
        ...
        onInput,
        {
            textareaHeightKey: positiveTextareaHeightKey,
            sizeKey: positiveChipHeightKey,
            onManualResize: (detail) => markPromptInnerManualResize(detail),
        },
    );
    ...
    const negative = createPromptRow(
        ...
        onInput,
        {
            textareaHeightKey: negativeTextareaHeightKey,
            sizeKey: negativeChipHeightKey,
            collapseKey: negativeCollapsedKey,
            collapsible: true,
            onManualResize: (detail) => markPromptInnerManualResize(detail),
        },
    );
    ...
    const regionalCanvasAutoInput = el("input", {
        type: "checkbox",
        title: "自动跟随当前工作流的生成图片/latent 画布尺寸；编辑宽高会退出自动跟随",
    });
    ...
    const regionalCanvasWidthInput = el("input", {
        ...
        title: "画布宽；输入后同步到实际 Image Size / Resize 节点，编辑时会退出自动跟随",
    });
    const regionalCanvasHeightInput = el("input", {
        ...
        title: "画布高；输入后同步到实际 Image Size / Resize 节点，编辑时会退出自动跟随",
    });
    ...
    const regionalDetectButton = el("button", {
        ...
        type: "button",
        title: "检测当前工作流的生成图片/latent 画布尺寸，并回到自动读取",
        onclick: () => {
    ...
    const refreshBridgeData = async (options = {}) => {
        ...
        setBridgeLowZoomThreshold(node, state.settings?.zoom_summary_threshold);
    ...
    const showWebUIIntegrationDialog = async () => {
        ...
        let autoDetectButton = null;
        let connectButton = null;
        let dialogBusy = false;
        let integrationOperationId = 0;
        let activeIntegrationController = null;
        const setDialogBusy = (busy, activeButton = null) => {
            dialogBusy = busy;
            rootInput.disabled = busy;
            guessSelect.disabled = busy;
            if (autoDetectButton) autoDetectButton.disabled = busy;
            if (connectButton) connectButton.disabled = busy;
            if (webuiAssetsButton) webuiAssetsButton.disabled = busy || Boolean(webuiAssetsButton.__webuiBridgeReady);
            if (autoDetectButton) autoDetectButton.textContent = busy && activeButton === autoDetectButton ? "检测中..." : "自动检测";
            if (connectButton) connectButton.textContent = busy && activeButton === connectButton ? "接入中..." : "接入并刷新";
        };
        const close = () => {
            activeIntegrationController?.abort?.();
        ...
        const renderInfo = (info) => {
            ...
            if (webuiAssetsButton) {
                webuiAssetsButton.__webuiBridgeReady = ready;
                webuiAssetsButton.disabled = dialogBusy || ready;
        ...
        const connect = async (autoDetect = false) => {
            ...
            activeIntegrationController = controller;
            const operationId = ++integrationOperationId;
            let root = rootInput.value.trim();
            statusLine.textContent = autoDetect ? "正在自动检测并接入..." : "正在接入 WebUI...";
            setDialogBusy(true, autoDetect ? autoDetectButton : connectButton);
            const isStale = () => controller.signal.aborted || operationId !== integrationOperationId;
            try {
                if (autoDetect && !root) {
                    const latest = await fetchWebUIIntegration({ signal: controller.signal });
                    if (isStale()) return;
                    renderInfo(latest);
                ...
                const data = await refreshBridgeData({
                    loraDetail: "basic",
                    loraTimeoutMs: 8000,
                    refreshTimeoutMs: WEBUI_REFRESH_TIMEOUT_MS,
                ...
                if (operationId === integrationOperationId) {
                    activeIntegrationController = null;
                    setDialogBusy(false);
                }
            }
        };
        const installWebUIAssets = async () => {
            ...
            setDialogBusy(true, webuiAssetsButton);
            const isStale = () => controller.signal.aborted || operationId !== integrationOperationId;
            try {
                if (!root) {
                    const latest = await fetchWebUIIntegration({ signal: controller.signal });
                ...
                const latestInfo = await fetchWebUIIntegration({ signal: controller.signal });
                if (isStale()) return;
                renderInfo(latestInfo);
                await refreshBridgeData({ signal: controller.signal });
                if (isStale()) return;
        ...
        mask.append(el("div", { class: "webui-bridge-config-panel" }, [
            ...
            el("div", { class: "webui-bridge-config-actions" }, [
                autoDetectButton = el("button", { type: "button", onclick: () => connect(true) }, "自动检测"),
                webuiAssetsButton = el("button", { type: "button", onclick: installWebUIAssets }, "补齐缺失扩展"),
                connectButton = el("button", { type: "button", class: "primary", onclick: () => connect(false) }, "接入并刷新"),
            ]),
        ]));
        document.body.append(mask);
        const initialOperationId = integrationOperationId;
        try {
            const initialInfo = await fetchWebUIIntegration();
            if (initialOperationId === integrationOperationId && !dialogBusy) {
                renderInfo(initialInfo);
            }
        } catch (error) {
            if (initialOperationId === integrationOperationId && !dialogBusy) {
    ...
    const applyLayoutPreset = (preset, options = {}) => {
        const normalizedPreset = rememberBridgeLayoutPreset(setLayoutPresetClass(preset));
        node.__webuiBridgeInitialLayoutPreset = normalizedPreset;
        ...
        const setPanelHeights = ({ top, lora, positiveText, positiveTags } = {}) => {
            requestAnimationFrame(() => {
                ...
                if (lora === null) {
                    ...
                    setResizeTargetHeight(extraSection, extraHeightKey, lora, { min: EXTRA_NETWORKS_MIN_HEIGHT, max: getExtraResizeMaxHeight() });
        ...
        if (normalizedPreset === "compact") {
            ...
            setPanelHeights({ top: 420, lora: COMPACT_EXTRA_INITIAL_HEIGHT, positiveText: 88, positiveTags: 150 });
        ...
        requestAnimationFrame(() => panel?.__webuiBridgeSettleManualResizeLayout?.());
    };
    ...
    const showImportTagsDialog = (options = {}) => {
        ...
        const initialKind = ["positive", "negative"].includes(options.kind) ? options.kind : "all";
        const managedSmallNodeLibrary = Boolean(options.smallNodeLibrary);
        ...
        const filterKind = settingSelect(initialKind, [
            { value: "all", label: "全部" },
        ...
        const refreshCustomList = async () => {
            ...
            if (clearAllButton) clearAllButton.disabled = customItems.length === 0;
        ...
        const clearAllCustomTags = async () => {
            if (!customItems.length) {
                statusLine.textContent = "当前没有可清空的自定义/市场提示词";
                return;
            }
            if (!confirm(`确定清空全部 ${customItems.length} 条自定义/市场提示词吗？WebUI 原始词库和收藏不会被删除。`)) return;
            statusLine.textContent = "正在清空自定义/市场词库...";
            try {
                const result = await clearCustomTags();
                state.customTagCount = result.custom_tag_count || result.total || 0;
                clearEditor();
                await refreshBridgeData();
                refreshPromptOnlyLibraries();
                await refreshCustomList();
                statusLine.textContent = `已清空 ${result.removed || 0} 条自定义/市场提示词`;
                setStatus("自定义/市场词库已清空", { kind: "success" });
            } catch (error) {
                statusLine.textContent = `清空失败: ${error?.message || error}`;
            }
        };
        clearAllButton = el("button", {
            type: "button",
            class: "danger",
            onclick: clearAllCustomTags,
            disabled: customItems.length ? undefined : "disabled",
        }, "清空自定义/市场");
        mask.addEventListener("mousedown", (event) => {
            if (event.target === mask) close();
        });
        mask.append(el("div", { class: "webui-bridge-config-panel webui-bridge-prompt-editor-panel" }, [
            el("div", { class: "webui-bridge-config-head" }, [
                el("span", {}, managedSmallNodeLibrary
                    ? `${initialKind === "negative" ? "负向" : initialKind === "positive" ? "正向" : "全部"}小节点提示词库管理`
                    : editorMode ? "提示词编辑" : "导入词库"),
                el("button", { type: "button", onclick: close }, "×"),
            ]),
            el("div", { class: "webui-bridge-config-body" }, [
                el("div", { class: "webui-bridge-config-note" }, managedSmallNodeLibrary
                    ? "这里管理正向、负向提示词小节点共用的分类词库。新增、修改、删除或导入后，画布上已展开的小节点会立即刷新；主页面与小节点共享同一份数据。"
                    : editorMode
    ...
    const showBridgeSettingsDialog = () => {
        ...
        const libraryStatusText = () => {
            const promptSource = librarySourceLabel(state.promptLibrary?.prompt?.source);
            const autocompleteSource = librarySourceLabel(state.promptLibrary?.autocomplete?.source);
            return `当前分类: ${promptSource}；全库搜索: ${autocompleteSource}`;
        };
        const libraryStatusLine = el("div", { class: "webui-bridge-config-note" }, libraryStatusText());
        ...
        const zoomSummaryThresholdInput = el("input", {
            class: "webui-bridge-config-input",
            type: "number",
            min: "0",
            max: "100",
            step: "1",
            title: "画布缩放低于这个百分比时显示摘要；设为 0 可关闭摘要模式",
        });
        zoomSummaryThresholdInput.value = String(Math.round(current.zoom_summary_threshold * 100));
        ...
        const downloadLocalAssets = async () => {
            ...
            try {
                ...
                await refreshBridgeData();
                refreshPromptOnlyLibraries();
                libraryStatusLine.textContent = libraryStatusText();
        ...
        const save = async () => {
            ...
            try {
                const zoomThresholdPercent = Number.parseFloat(zoomSummaryThresholdInput.value);
                const zoomSummaryThreshold = normalizeBridgeLowZoomThreshold(
                    Number.isFinite(zoomThresholdPercent) ? zoomThresholdPercent / 100 : current.zoom_summary_threshold,
                );
                const result = await saveBridgeSettings({
                    ...
                    zoom_summary_threshold: zoomSummaryThreshold,
                    show_startup_wizard: wizardToggle.checked,
                    ui_visibility: collectVisibilitySettings(),
                });
                state.settings = normalizeBridgeSettings(result.settings);
                setGraphBridgeLowZoomThreshold(state.settings.zoom_summary_threshold);
                state.customTagCount = result.custom_tag_count || state.customTagCount;
            ...
            el("div", { class: "webui-bridge-config-body" }, [
                ...
                el("div", { class: "webui-bridge-settings-grid" }, [
                    ...
                    configSection("布局 / 显示", "调节点内部密度，不影响工作流执行。", [
                        ...
                        el("label", {}, [el("span", {}, "远景摘要阈值 (%)"), zoomSummaryThresholdInput]),
                        el("label", { class: "webui-bridge-config-check" }, [
                            wizardToggle,
                            el("span", {}, "首次向导"),
                        ]),
                    ...
                    configSection("全局提示词库与市场", "主节点与正/负向小节点共用同一份词库；下方的正向、负向按钮只是筛选视图，编辑、删除或市场导入会同步到所有提示词面板。", [
                        el("div", { class: "webui-bridge-settings-asset-tools" }, [
                            el("button", {
                                type: "button",
                                onclick: () => showImportTagsDialog({ mode: "editor", kind: "positive", smallNodeLibrary: true }),
                            }, "管理正向词库"),
                            el("button", {
                                type: "button",
                                onclick: () => showImportTagsDialog({ mode: "editor", kind: "negative", smallNodeLibrary: true }),
                            }, "管理负向词库"),
                            el("button", {
                                type: "button",
    ...
    const renderTree = () => {
        ...
        const renderNode = (node, depth = 0) => {
            ...
            const row = el("button", {
                ...
                title: loraTreeNodeTitle(node, isRoot),
    ...
    const renderCards = () => {
        ...
        const listSignature = [
            ...
            filter,
            state.loraSort,
            state.loraSortDescending ? "desc" : "asc",
            state.loraPageSize,
            state.loraPage,
        ].join("|");
        const shouldRestoreScroll = state.__webuiBridgeLoraListSignature === undefined ||
            state.__webuiBridgeLoraListSignature === listSignature;
        const preservedScrollTop = shouldRestoreScroll
            ? (cards.scrollTop || readLocalNumber(loraScrollTopKey, 0))
            : 0;
        ...
        window.requestAnimationFrame?.(() => {
            ...
            cards.__webuiBridgeRestoringScroll = true;
            cards.scrollTop = Math.max(0, preservedScrollTop || 0);
            window.setTimeout?.(() => {
                cards.__webuiBridgeRestoringScroll = false;
            }, 0);
        });
        scheduleAdaptiveLayout();
    };
    ...
    const setNodeSize = (width, height, options = {}) => {
        ...
        syncNodeSizeInputs();
        if (markChanged && (rememberSize || userSize)) markGraphChanged(node);
    ...
    let promptInnerSplitManual = [
        positiveTextareaHeightKey,
        negativeTextareaHeightKey,
        positiveChipHeightKey,
        negativeChipHeightKey,
        positiveTagHeightKey,
        negativeTagHeightKey,
    ].some((key) => readLocalNumber(key, null) !== null);
    ...
    const holdNodeBottomFit = () => {
        ...
        schedulePostSuppressionAdaptiveLayout();
    };
    ...
    markPromptInnerManualResize = ({ commit = false } = {}) => {
        promptInnerSplitManual = true;
        holdPromptInnerFit();
        holdPromptSplitFit();
        if (commit) markGraphChanged(node);
    };
    ...
    const endManualNodeResize = ({ changed = false } = {}) => {
        manualNodeResizeActive = false;
        holdPanelAutoFit();
        if (changed) {
            if (!resizeTargetHidden(topRow)) persistResizeTargetHeight(topRow, topRowHeightKey);
            if (!resizeTargetHidden(extraSection)) persistResizeTargetHeight(extraSection, extraHeightKey);
            markGraphChanged(node);
        }
        scheduleManualResizeSettle();
    };
    ...
    const installResizeDrag = (handle) => {
        handle.addEventListener("pointerdown", (event) => {
            ...
            const onMove = (moveEvent) => {
                ...
                moved = moved || Math.abs(deltaWidth) >= 1 || Math.abs(deltaHeight) >= 1;
                setNodeSize(startWidth + deltaWidth, startHeight + deltaHeight, { user: true, markChanged: false });
                resizePanelSectionsWithNode(deltaHeight, startSections, { persist: false });
            };
            const onUp = () => {
                handle.classList.remove("dragging");
                endManualNodeResize({ changed: moved });
                document.removeEventListener("pointermove", onMove, true);
                document.removeEventListener("pointerup", onUp, true);
                document.removeEventListener("pointercancel", onUp, true);
            };
    ...
    const installNodeWidthDrag = (handle) => {
        handle.addEventListener("pointerdown", (event) => {
            ...
            const onMove = (moveEvent) => {
                ...
                moved = true;
                holdPanelAutoFit();
                setNodeSize(startWidth + deltaWidth, startHeight, { user: true, markChanged: false });
            };
            const onUp = () => {
                handle.classList.remove("dragging");
                endManualNodeResize({ changed: moved });
    ...
    const sizeControls = el("div", { class: "webui-bridge-size-controls" }, [
        nodeSizeField("节点宽", nodeWidthInput),
        nodeSizeField("节点高", nodeHeightInput),
        el("button", { title: "窄版尺寸", onclick: () => setNodeSize(PANEL_MIN_WIDTH, 620, { user: true }) }, "S"),
    ...
    const promptPolarityGrip = createHeightSplitGrip(
            ...
            () => negative.textarea.__webuiBridgeHeightKey,
            {
                ...
                onDragEnd: () => {
                    markPromptInnerManualResize({ commit: true });
                    holdPromptInnerFit();
                    holdPromptSplitFit();
                },
            },
    ...
    const ensureUsableExtraHeight = (preferredHeight = EXTRA_NETWORKS_USABLE_MIN_HEIGHT) => {
        ...
        const maxExtraHeight = getExtraResizeMaxHeight();
    ...
    resizeExtraWithNode = (deltaHeight, { persist = true } = {}) => {
    ...
    settleManualResizeLayout = ({ automatic = false } = {}) => {
        ...
        window.requestAnimationFrame?.(() => {
            window.requestAnimationFrame?.(() => {
                ...
                const settleTolerance = Math.max(12 * scale, panelRect.height * 0.01);
                if (Math.abs(blank) <= settleTolerance) return;
                const currentHeight = resizeTargetLayoutHeight(extraSection) || EXTRA_NETWORKS_MIN_HEIGHT;
                let nextExtraHeight = currentHeight + blank / scale;
                const extraMin = EXTRA_NETWORKS_MIN_HEIGHT;
                const extraMax = getExtraResizeMaxHeight();
                const currentTopHeight = topRow?.isConnected ? (resizeTargetLayoutHeight(topRow) || 0) : 0;
                const topComfortHeight = topRow?.isConnected ? getTopRowComfortHeight() : 0;
                const topOvershoot = currentTopHeight && topComfortHeight
                    ? currentTopHeight - topComfortHeight
                    : 0;
                if (topOvershoot > Math.max(48, settleTolerance / scale) && blank >= -settleTolerance) {
                    const blankLayout = blank / scale;
                    const maxTopTransfer = Math.max(0, extraMax - currentHeight - blankLayout);
                    const topReduction = Math.min(topOvershoot, maxTopTransfer);
                    if (topReduction > 1) {
                    ...
                    setResizeTargetHeight(extraSection, extraHeightKey, currentHeight + blankLayout + topReduction, {
                        min: extraMin,
                        max: extraMax,
                    });
                    extraSection.style.flex = "0 0 auto";
                ...
                } else if (blank > 0 && topRow?.isConnected) {
                    const cappedExtraHeight = clampNumber(nextExtraHeight, extraMin, extraMax);
                    const remainingBlank = Math.max(0, blank / scale - (cappedExtraHeight - currentHeight));
                    if (remainingBlank > 1) {
                        const topMax = Math.min(
                            topRow.__webuiBridgeMaxHeight || PANEL_MAX_HEIGHT,
                            topComfortHeight || currentTopHeight,
                        );
                        const topIncrease = Math.min(Math.max(0, topMax - currentTopHeight), remainingBlank);
                        if (topIncrease > 1) {
                            setResizeTargetHeight(topRow, topRowHeightKey, currentTopHeight + topIncrease, {
                                min: getTopRowVisibleMinHeight(),
                                max: topMax,
                            });
                        }
    ...
    function applyExtraCollapsedState(options = {}) {
        ...
        if (preserveExpandedHeight && preservedHeight) {
            ...
            applyStoredHeight(extraSection, extraHeightKey, { min: EXTRA_NETWORKS_MIN_HEIGHT, max: getExtraResizeMaxHeight() });
        }
        extraSection.style.flex = "0 0 auto";
    ...
    function getExtraResizeMaxHeight() {
        const inlineMax = extraSection?.__webuiBridgeMaxHeight || EXTRA_NETWORKS_INLINE_MAX_HEIGHT;
        return negative.row?.classList?.contains("collapsed")
            ? Math.max(inlineMax, EXTRA_NETWORKS_MAX_HEIGHT)
            : inlineMax;
    }
    ...
    const seedFreshInitialLayout = () => {
        ...
        const seededPreset = normalizeBridgeLayoutPreset(
            node.__webuiBridgeInitialLayoutPreset || state.settings?.layout_preset,
        );
        const seededSections = BRIDGE_FRESH_SECTION_SIZE_BY_PRESET[seededPreset] || BRIDGE_FRESH_SECTION_SIZE_BY_PRESET.compact;
        setLayoutPresetClass(seededPreset);
        promptLoraSplitManual = true;
        setResizeTargetHeight(topRow, topRowHeightKey, seededSections.top, {
            min: getTopRowVisibleMinHeight(),
            max: topRow.__webuiBridgeMaxHeight,
        });
        setResizeTargetHeight(extraSection, extraHeightKey, Math.max(seededSections.extra, getExtraVisibleFloor()), {
            min: getExtraVisibleFloor(),
            max: getExtraResizeMaxHeight(),
        });
        extraSection.style.flex = "0 0 auto";
    ...
    function visibleStackContentHeight(container) {
        ...
        const height = children.reduce((total, child) => total + resizeTargetLayoutHeight(child), 0) +
            gap * Math.max(0, children.length - 1);
        return Math.ceil(height);
    }
    function getTopRowComfortHeight() {
        const topMin = getTopRowVisibleMinHeight();
        const topMax = getTopRowVisibleMaxHeight();
        const promptContentHeight = visibleStackContentHeight(promptsColumn);
        const actionContentHeight = actionCollapsed ? 0 : visibleStackContentHeight(actionColumn);
        const sideGripHeight = actionCollapsed ? 0 : resizeTargetLayoutHeight(actionResizeGrip);
        const contentHeight = Math.max(promptContentHeight, actionContentHeight, sideGripHeight, topMin);
        return clampNumber(contentHeight, topMin, topMax);
    }
    resizePanelSectionsWithNode = (deltaHeight, startSections = capturePanelSectionHeights(), { persist = true } = {}) => {
        if (!deltaHeight || extraCollapsed || loraOverlayOpen || !topRow?.isConnected || !extraSection?.isConnected) {
            resizeExtraWithNode(deltaHeight, { persist });
            return;
        }
        promptLoraSplitManual = true;
        holdPanelAutoFit();
    ...
    panelLayoutIsMeasurable = () => {
        ...
        return Boolean(
            ...
            rect.height > 8 &&
            panel.offsetWidth > 8 &&
            panel.offsetHeight > 8
        );
    };
    const layoutHeightForState = (target, storageKey = "") => {
        const height = resizeTargetLayoutHeight(target) || Math.round(target?.getBoundingClientRect?.().height || 0);
        if (height) return height;
        return storageKey ? readLocalNumber(storageKey, null) : null;
    };
    const collectBridgeLayoutState = () => {
        if (!panelLayoutIsMeasurable()) {
            return lastVisibleBridgeLayoutState
                ? { ...lastVisibleBridgeLayoutState }
                : normalizeBridgeLayoutState(node.__webuiBridgeConfiguredLayoutState);
        }
        const sidebarWidth = topRow
        ...
        const layoutState = normalizeBridgeLayoutState({
            ...
            sidebar_collapsed: Boolean(actionCollapsed),
            negative_collapsed: Boolean(negative.row?.classList?.contains("collapsed")),
            positive_textarea_height: layoutHeightForState(positive.textarea, positiveTextareaHeightKey),
            negative_textarea_height: layoutHeightForState(negative.textarea, negativeTextareaHeightKey),
            positive_chip_height: layoutHeightForState(positive.row?.chips, positiveChipHeightKey),
            negative_chip_height: layoutHeightForState(negative.row?.chips, negativeChipHeightKey),
            positive_tag_height: layoutHeightForState(positiveTagPanel, positiveTagHeightKey),
            negative_tag_height: layoutHeightForState(negativeTagPanel, negativeTagHeightKey),
            lora_scroll_top: cards?.isConnected ? Math.max(0, Math.round(cards.scrollTop || 0)) : readLocalNumber(loraScrollTopKey, null),
        });
        if (layoutState) lastVisibleBridgeLayoutState = { ...layoutState };
        return layoutState;
    };
    panel.__webuiBridgeGetLayoutState = collectBridgeLayoutState;
    panel.__webuiBridgeMarkPromptInnerManualResize = markPromptInnerManualResize;
    panel.__webuiBridgeHandleSectionResizeEnd = ({ target, before, after, grip } = {}) => {
        const promptResize = Boolean(
            target?.closest?.(".webui-bridge-prompts") ||
            before?.closest?.(".webui-bridge-prompts") ||
            after?.closest?.(".webui-bridge-prompts") ||
            grip?.closest?.(".webui-bridge-prompts")
        );
        if (promptResize) {
            markPromptInnerManualResize({ commit: true });
            return;
    ...
    let loraScrollSaveTimer = 0;
    cards.addEventListener("scroll", () => {
        if (!cards.isConnected) return;
        if (cards.__webuiBridgeRestoringScroll) return;
        window.clearTimeout?.(loraScrollSaveTimer);
        loraScrollSaveTimer = window.setTimeout?.(() => {
            writeLocalNumber(loraScrollTopKey, Math.max(0, Math.round(cards.scrollTop || 0)));
            markGraphChanged(node);
        }, 160) || 0;
    }, { passive: true });
    ...
    const fitLoraSectionToVisibleRows = (rows = 1, { force = false, fillAvailable = false } = {}) => {
        ...
        const shouldFillAvailable = hasLargeBottomBlank || (fillAvailable &&
            !promptLoraSplitManual && !promptInnerSplitManual);
    ...
    const fitPromptColumnToContent = () => {
        ...
        if (textareaTarget && !resizeTargetHidden(textareaTarget)) {
            ...
            if (!hasManualTextHeight && !promptInnerManualActive) {
        ...
        if (chipTarget && !resizeTargetHidden(chipTarget) && !chipTarget.classList.contains("empty") &&
            !promptInnerManualActive &&
            readLocalNumber(chipTarget.__webuiBridgeHeightKey, null) === null) {
    ...
    loadBridgeData().then((data) => {
        panel.__webuiBridgeDataLoaded = true;
        state.loras = data.loras;
        state.styles = data.styles;
        state.models = data.models;
        state.promptAllInOne = data.promptAllInOne;
        ...
        if (!node.__webuiBridgeInitialLayoutApplied) {
            ...
            if (shouldApplyFreshDefaults) {
                const initialPreset = normalizeBridgeLayoutPreset(state.settings?.layout_preset);
                const presetSize = freshBridgePanelSizeForPreset(initialPreset);
                applyLayoutPreset(initialPreset, {
                    preserveNodeSize: bridgePanelSizesMatch(node.size, presetSize, 1),
                });
            } else {
...
function installWebUIPanel(node) {
    ...
    if (node.__webuiBridgePanel) {
        ...
        const needsConfiguredLayoutRebuild = hasBridgeLayoutState(node.__webuiBridgeConfiguredLayoutState) &&
            !node.__webuiBridgeConfiguredLayoutStateApplied;
        if (usablePanel && !needsConfiguredLayoutRebuild) {
            ...
            if (shouldRepairBridgePanelSize(node.size) && !hasBridgeLayoutState(node.__webuiBridgeConfiguredLayoutState)) {
    ...
    const hadRepairablePanelSize = shouldRepairBridgePanelSize(node.size);
    let panel = null;
    try {
    ...
    if (!node.__webuiBridgeSerializeWrapped) {
        chainCallback(node, "onSerialize", function (data) {
            ...
            data.size = [...desired];
            if (layoutState) {
                data.properties = data.properties && typeof data.properties === "object" ? { ...data.properties } : {};
                data.properties[BRIDGE_LAYOUT_STATE_PROPERTY] = layoutState;
                node.__webuiBridgeConfiguredLayoutState = layoutState;
            }
    ...
    if (nodeHasSavedUserBridgePanelSize(node)) {
        ...
        node.__webuiBridgeDesiredSize = node.__webuiBridgeFreshNode && !node.__webuiBridgeWasConfigured
            ...
            : looksLikeLegacyDefaultSize(node.size)
                ? clampPanelSize(DEFAULT_PANEL_WIDTH, DEFAULT_PANEL_HEIGHT)
            : shouldRepairBridgePanelSize(node.size)
                ? clampPanelSize(DEFAULT_PANEL_WIDTH, DEFAULT_PANEL_HEIGHT)
            : normalizeBridgePanelSize(node.size);
    } else if (shouldRepairBridgePanelSize(node.__webuiBridgeDesiredSize)) {
        node.__webuiBridgeDesiredSize = shouldRepairBridgePanelSize(node.size)
            ? clampPanelSize(DEFAULT_PANEL_WIDTH, DEFAULT_PANEL_HEIGHT)
            : normalizeBridgePanelSize(node.size);
    }
    ...
    const domWidget = node.addDOMWidget("webui_prompt_frontend", "webui_prompt_frontend", panel, {
        ...
        hideOnZoom: false,
        getMinHeight: () => 280,
        getMaxHeight: () => Math.max(280, PANEL_MAX_HEIGHT - DOM_WIDGET_PANEL_PAD),
    });
    installWidgetSerializationFallback(domWidget, () => null);
    ...
    const applyDomWidgetSize = (nodeWidth, nodeHeight) => {
        ...
        panel.style.width = `${widgetWidth}px`;
        panel.style.height = `${widgetHeight}px`;
        syncBridgeDomWidgetWrapper(panel, widgetWidth, widgetHeight);
        window.requestAnimationFrame?.(() => syncBridgeDomWidgetWrapper(panel, widgetWidth, widgetHeight));
        return [widgetWidth, widgetHeight];
    };
    ...
    node.resizable = true;
    if (nodeHasSavedUserBridgePanelSize(node)) {
        const savedSize = resolveBridgePanelSizeForLiveState(node, { clearConfiguredLock: true });
        const currentWidth = Math.round(Number(node.size?.[0] || 0));
        const currentHeight = Math.round(Number(node.size?.[1] || 0));
    ...
    if ((hadRepairablePanelSize || node.__webuiBridgeRepairedPanelSize) &&
        !hasBridgeLayoutState(node.__webuiBridgeConfiguredLayoutState)) {
        window.requestAnimationFrame?.(() => {
...
function addStyles() {
    ...
    style.textContent = `
        ...
        .lg-node-widget.webui-bridge-dom-widget-shell {
        ...
        .webui-bridge-size-controls {
            display: grid;
            grid-template-columns: repeat(2, minmax(92px, 1fr)) 32px 32px 32px minmax(56px, 1fr);
            gap: 5px;
        }
        ...
        .webui-bridge-aio-body.drop-target {
        ...
        .webui-bridge-tab-add:hover {
            opacity: 1;
            background: #2a3648 !important;
            border-style: solid;
        }
        .webui-bridge-aio-tabs button.drop-target,
        .webui-bridge-aio-subtabs button.drop-target {
            background: rgba(106, 163, 255, 0.3) !important;
            box-shadow: inset 0 0 0 2px #6aa3ff;
        }
        .webui-bridge-aio-tabs button.dragging {
            opacity: 0.5;
        }
        .webui-bridge-aio-tabs button.webui-bridge-tab-insert-before {
            border-left: 3px solid #6aa3ff !important;
        }
        .webui-bridge-aio-tabs button.webui-bridge-tab-insert-after {
            border-right: 3px solid #6aa3ff !important;
        }
        .webui-bridge-aio-subtabs button.dragging {
            opacity: 0.5;
        }
        .webui-bridge-aio-subtabs button.webui-bridge-tab-insert-before {
            border-left: 3px solid #6aa3ff !important;
        }
        .webui-bridge-aio-subtabs button.webui-bridge-tab-insert-after {
            border-right: 3px solid #6aa3ff !important;
        }
        .webui-bridge-tab-popup {
            background: #1a2332;
        ...
        .webui-bridge-tab-popup-item:hover {
            background: #2a3648;
        }
        .webui-bridge-tab-popup-danger {
            color: #ff8a8a;
        }
        ...
        .webui-bridge-regional-canvas-row {
            grid-column: 1 / -1;
            display: grid !important;
            grid-template-columns: auto minmax(0, 1fr) auto minmax(0, 1fr) auto auto 28px;
            align-items: center;
            gap: 5px;
        }
        ...
        .webui-bridge-prompt-only-panel {
            box-sizing: border-box;
            display: flex;
            flex-direction: column;
            gap: 7px;
        ...
        .webui-bridge-prompt-only-label {
            display: flex;
            align-items: center;
            justify-content: space-between;
            gap: 8px;
        }
        .webui-bridge-prompt-only-head-actions {
            display: flex;
            align-items: center;
            justify-content: flex-end;
            gap: 7px;
        ...
        .webui-bridge-prompt-only-label span,
        .webui-bridge-prompt-only-help {
            color: #9da8b7;
            font-size: 11px;
        }
        .webui-bridge-prompt-only-library-toggle {
            min-width: 82px;
            height: 25px;
            padding: 0 8px;
        ...
        .webui-bridge-prompt-only-settings,
        .webui-bridge-prompt-only-lora-button {
            flex: 0 0 25px;
            height: 25px;
            padding: 0;
            border: 1px solid #4b596c;
        ...
        .webui-bridge-prompt-only-settings:hover,
        .webui-bridge-prompt-only-lora-button:hover {
            border-color: #6aa3ff;
            background: #274b7c;
            color: #fff;
        }
        .webui-bridge-prompt-only-library-toggle:hover,
        .webui-bridge-prompt-only-library-toggle.active {
            border-color: #6aa3ff;
            background: #274b7c;
            color: #fff;
        }
        .webui-bridge-prompt-only-textarea {
            box-sizing: border-box;
            flex: 1 1 auto;
            width: 100%;
            min-height: 118px;
            ...
            background: #11161d;
            color: #f2f4f7;
            font: 12px/1.5 Consolas, "Microsoft YaHei UI", sans-serif;
        }
        .webui-bridge-prompt-only-textarea:focus {
            border-color: #5c9ded;
            box-shadow: 0 0 0 1px rgba(92, 157, 237, 0.28);
        }
        .webui-bridge-prompt-only-textarea[hidden],
        .webui-bridge-prompt-only-chips[hidden] {
            display: none !important;
        }
        ...
        .webui-bridge-prompt-only-chips:focus-within {
            border-color: #5c9ded;
            box-shadow: 0 0 0 1px rgba(92, 157, 237, 0.28);
        }
        .webui-bridge-prompt-only-chip {
            position: relative;
            display: flex;
            flex: 0 1 auto;
            min-width: 74px;
        ...
        .webui-bridge-prompt-only-chip.dragging {
            opacity: .45;
            cursor: grabbing;
        }
        .webui-bridge-prompt-only-chip.drop-before {
            box-shadow: -4px 0 0 #6aa3ff;
        }
        .webui-bridge-prompt-only-chip.drop-after {
            box-shadow: 4px 0 0 #6aa3ff;
        }
        .webui-bridge-prompt-only-chip:hover,
        .webui-bridge-prompt-only-chip.active {
            border-color: #6aa3ff;
            background: #303a4d;
        }
        .webui-bridge-prompt-only-chip.lora {
            border-color: #b27816;
            background: #352a19;
        }
        .webui-bridge-prompt-only-chip-content {
            display: flex;
            flex: 1 1 auto;
            flex-direction: column;
            min-width: 0;
        ...
        .webui-bridge-prompt-only-chip-remove {
            position: absolute;
            top: 2px;
            right: 2px;
            width: 18px;
        ...
        .webui-bridge-prompt-only-chip-remove:hover {
            background: #71343b;
            color: #fff;
        }
        .webui-bridge-prompt-only-chip-separator {
            position: absolute;
            top: 2px;
            right: 21px;
            width: 18px;
        ...
        .webui-bridge-prompt-only-chip-separator:hover,
        ...
        .webui-bridge-prompt-only-separator-marker {
            ...
            height: 40px;
            padding: 0;
            border: 1px dashed #4f779f;
            border-radius: 5px;
            background: rgba(51, 96, 139, .18);
            color: #8ec4f4;
            cursor: pointer;
            font: 800 14px/38px sans-serif;
        }
        .webui-bridge-prompt-only-separator-marker:hover {
            border-color: #7fc2ff;
            background: rgba(64, 125, 184, .3);
            color: #fff;
        }
        .webui-bridge-prompt-only-chip-main,
        .webui-bridge-prompt-only-chip-local {
            box-sizing: border-box;
            display: block;
            width: 100%;
            padding: 3px 7px;
        ...
        .webui-bridge-prompt-only-chip-main {
            color: #f2f5fb;
            font: 700 11px/15px Consolas, Monaco, monospace;
        }
        .webui-bridge-prompt-only-chip-local {
            color: #e5d7ae;
            background: rgba(184, 145, 60, .12);
            font-size: 11px;
            line-height: 15px;
        }
        .webui-bridge-prompt-only-chunk-input {
            box-sizing: border-box;
            flex: 1 1 150px;
            min-width: 130px;
            height: 40px;
            padding: 0 8px;
            border: 1px dashed #46556a;
            border-radius: 5px;
            outline: none;
            background: #181f29;
            color: #f2f4f7;
            font: 12px/38px Consolas, "Microsoft YaHei UI", sans-serif;
        }
        .webui-bridge-prompt-only-chunk-input:focus {
            border-color: #6aa3ff;
            border-style: solid;
            background: #1c2633;
        }
        .webui-bridge-prompt-only-lora-picker {
            width: min(920px, calc(100vw - 48px));
            max-width: 920px;
            height: min(860px, calc(100vh - 48px));
        }
        .webui-bridge-prompt-only-lora-picker .webui-bridge-config-body {
            min-height: 0;
            overflow: hidden;
            grid-template-rows: auto auto auto minmax(0, 1fr);
        }
        .webui-bridge-prompt-only-lora-toolbar {
            display: grid;
            grid-template-columns: minmax(0, 1fr) minmax(190px, .42fr) auto;
            gap: 10px;
            align-items: end;
        }
        .webui-bridge-prompt-only-lora-toolbar label {
            display: grid;
            gap: 7px;
            align-items: center;
            color: #b9c5d6;
        }
        .webui-bridge-prompt-only-lora-filter-control {
            grid-template-columns: auto minmax(140px, 1fr);
        }
        .webui-bridge-prompt-only-lora-weight-control {
            grid-template-columns: auto 78px;
        }
        .webui-bridge-prompt-only-lora-filter {
            min-width: 0;
        }
        .webui-bridge-prompt-only-lora-weight {
            width: 78px;
        }
        .webui-bridge-prompt-only-lora-grid {
            display: grid;
        ...
        .webui-bridge-prompt-only-lora-card {
            display: grid;
        ...
        .webui-bridge-prompt-only-lora-card:hover,
        .webui-bridge-prompt-only-lora-card.added {
            border-color: #6aa3ff;
            background: #2a3d57;
        }
        .webui-bridge-prompt-only-lora-card.adding {
            opacity: .62;
            pointer-events: none;
        }
        .webui-bridge-prompt-only-lora-card img,
        .webui-bridge-prompt-only-lora-fallback {
            display: grid;
            place-items: center;
            width: 100%;
            height: 110px;
        ...
        .webui-bridge-prompt-only-lora-name,
        .webui-bridge-prompt-only-lora-folder {
            padding: 4px 7px;
            overflow: hidden;
            text-overflow: ellipsis;
            white-space: nowrap;
        ...
        .webui-bridge-prompt-only-library[hidden] {
            display: none !important;
        }
        .webui-bridge-prompt-only-library {
            flex: 1 1 auto;
            min-width: 0;
            min-height: 240px;
            overflow: hidden;
        }
        .webui-bridge-prompt-only-panel.library-expanded .webui-bridge-prompt-only-textarea {
            flex: 0 0 clamp(96px, 24%, 190px);
            min-height: 76px;
            resize: vertical;
        }
        .webui-bridge-prompt-only-panel.library-expanded .webui-bridge-prompt-only-chips {
            flex: 0 0 clamp(96px, 24%, 190px);
            min-height: 76px;
        }
        .webui-bridge-prompt-only-aio {
            width: 100%;
            height: 100% !important;
            min-height: 0 !important;
        ...
        .webui-bridge-prompt-only-aio > .webui-bridge-section-resizer {
            display: none;
        }
        .webui-bridge-prompt-only-loading {
            display: grid;
            place-items: center;
            height: 100%;
            min-height: 180px;
            border: 1px dashed #3f5067;
            border-radius: 6px;
            color: #aebbd0;
            background: #111722;
        }
        .webui-bridge-prompt-only-loading.error {
            color: #ffadb4;
        }
        @media (max-width: 720px) {
            ...
            .webui-bridge-prompt-only-head-actions > span {
                display: none;
            }
            .webui-bridge-prompt-only-lora-toolbar {
                grid-template-columns: minmax(0, 1fr) auto;
            }
            .webui-bridge-prompt-only-lora-toolbar > .webui-bridge-config-input:first-child {
                grid-column: 1 / -1;
            }
        }
        ...
        .webui-bridge-config-actions button.danger {
            border-color: #b95b64;
            background: #71333a;
        }
...
app.registerExtension({
    ...
    beforeRegisterNodeDef(nodeType, nodeData) {
        installWorkflowLoadSanitizer();
        const promptOnlyConfig = PROMPT_ONLY_NODES[nodeData.name];
        if (promptOnlyConfig) {
            chainCallback(nodeType.prototype, "onNodeCreated", function () {
                schedulePromptOnlyPanelInstall(this, promptOnlyConfig);
            });
            chainCallback(nodeType.prototype, "onConfigure", function (data) {
                schedulePromptOnlyPanelInstall(this, promptOnlyConfig, data);
            });
            return;
        }
        ...
        chainCallback(nodeType.prototype, "onNodeCreated", function () {
            ...
            this.__webuiBridgeInitialLayoutPreset = readBridgeInitialLayoutPreset();
            this.__webuiBridgeDesiredSize = freshBridgePanelSizeForPreset(this.__webuiBridgeInitialLayoutPreset);
            this.__webuiBridgeSavedPanelSize = null;
            this.__webuiBridgeConfiguredLayoutState = null;
            this.__webuiBridgeConfiguredLayoutStateApplied = false;
            this.size = [...this.__webuiBridgeDesiredSize];
            this.setSize?.([...this.__webuiBridgeDesiredSize]);
        ...
        chainCallback(nodeType.prototype, "onConfigure", function (data) {
            ...
            migrateLayoutStorage(BRIDGE_LAYOUT_MIGRATED_STORAGE_KEYS);
            this.__webuiBridgeConfiguredLayoutState = normalizeBridgeLayoutState(
                data?.properties?.[BRIDGE_LAYOUT_STATE_PROPERTY] ||
            ...
            if (looksLikeSavedUserBridgePanelSize(data?.size) || repairConfiguredSize) {
                ...
                if (Math.round(Number(this.size?.[0] || 0)) !== this.__webuiBridgeSavedPanelSize[0] ||
                    ...
                    try {
                        this.size = [...this.__webuiBridgeSavedPanelSize];
                        this.setSize?.([...this.__webuiBridgeSavedPanelSize]);
                    } finally {
                        this.__webuiBridgeApplyingConfiguredSize = false;
                    }
                }
            } else {
                this.__webuiBridgeSavedPanelSize = null;
        ...
        chainCallback(nodeType.prototype, "onDrawForeground", function (ctx) {
            const hasSlotLabelOverlay = Boolean(this.__webuiBridgeSlotLabelOverlay?.isConnected);
            if (!hasSlotLabelOverlay) drawBridgeExternalSlotLabels(this, ctx);
            if (isBridgePanelUsable(this)) {
                if (!hasSlotLabelOverlay) installBridgeSlotLabelOverlay(this);
                return;
            }
    ...
    afterConfigureGraph(missingNodes) {
        ...
        scheduleBridgePanelLayoutRepairForGraph("afterConfigureGraph");
    },
});
